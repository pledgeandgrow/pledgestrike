...
pub async fn connect(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if status == 200
        && (body.contains("stomp") || body.contains("STOMP") || body.contains("activemq"))
    {
...
pub async fn enumerate(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let enum_endpoints = [
        ...
        (
            "Stats",
            "/api/jolokia/read/org.apache.activemq:type=Broker,brokerName=*",
        ),
    ];
