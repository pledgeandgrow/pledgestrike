...
pub async fn expose(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if found.is_empty() {
        ...
        println!(
            "\n{} {} git file(s) exposed! Full repo may be recoverable.",
            "[!]".red().bold(),
            found.len()
        );
        println!(
            "{} Use 'pledgestrike git dump --url {}' to attempt full reconstruction.",
            "[*]".cyan().bold(),
            url
        );
    }
...
pub async fn dump(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if resp.status().as_u16() != 200 {
        println!(
            "{} .git/HEAD not accessible — cannot dump.",
            "[-]".red().bold()
        );
        return Ok(());
    }
    ...
    if resp.status().as_u16() == 200 {
        ...
        match client.get(&commit_url).send().await {
            Ok(r) if r.status().as_u16() == 200 => {
                println!(
                    "  {} Commit object accessible at {}",
                    "[+]".green().bold(),
                    commit_url
                );
            }
            _ => {
                println!("  {} Commit object not directly accessible", "*".yellow());
            }
        }
    }
    ...
    println!(
        "\n{} Use gitdumper.sh or similar to fully reconstruct.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn hook(url: &str, timeout: u64, token: Option<&str>) -> anyhow::Result<()> {
    ...
    for (hook_name, payload) in &hooks {
        ...
        match req.body(payload.to_string()).send().await {
            ...
            Err(_) => {
                println!("  {} {:20} error", "*".red(), hook_name);
            }
        }
    }
...
pub async fn actions(url: &str, timeout: u64, token: Option<&str>) -> anyhow::Result<()> {
        ...
        match req.send().await {
            Ok(resp) => {
                ...
                if status == 200 {
                    ...
                    let has_run_command = body.contains("run:")
                        && (body.contains("${{") || body.contains("github.event."));
                    let tag = "EXPOSED".red().bold().to_string();
                    println!(
                        "  {} {:40} {} — pull_request_target:{} secrets:{} injectable:{}",
                        "[!]".red().bold(),
                        path,
                        tag,
    ...
    let vectors = [
        (
            "pull_request_target",
            "Triggers workflow with write access on fork PRs",
        ),
        (
            "Issue/PR body injection",
            "${{ github.event.issue.body }} passed to run:",
        ),
        (
            "Branch name injection",
            "${{ github.event.ref }} in run: step",
        ),
        (
            "GITHUB_TOKEN abuse",
            "Token with repo: scope exposed to fork",
        ),
