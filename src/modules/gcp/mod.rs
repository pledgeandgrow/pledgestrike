...
const GCP_ENDPOINTS: &[(&str, &str)] = &[
    ...
    (
        ...
        "https://cloudbuild.googleapis.com/v1/projects/-/triggers",
    ),
    (
        "Artifact Registry",
...
pub async fn abuse(token: Option<&str>, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "\n{} Testing service account token scopes...",
        "[*]".cyan().bold()
    );
            ...
            if status == 200 {
                ...
                for scope in SA_SCOPES {
                    if body.contains(scope) {
                        ...
                        } else if scope.contains("full_control")
                            || scope.contains("read_write")
                        ...
                        println!(
                            "    {} scope: {} — {}",
                            ">".cyan(),
                            scope.split('/').next_back().unwrap_or(scope),
                            risk
                        );
                    }
                }
                ...
                println!(
                    "  {} Token info status={} — may need token parameter",
                    "*".yellow(),
                    status
                );
            }
    ...
    println!(
        "\n{} Testing IAM signBlob / generateAccessToken...",
        "[*]".cyan().bold()
    );
    let iam_endpoints = [
        (
