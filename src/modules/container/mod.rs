...
pub async fn docker(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if !exposed.is_empty() {
        println!(
            "\n{} Docker API is exposed! {} endpoints accessible.",
            "[!]".red().bold(),
            exposed.len()
        );
        ...
        println!(
            "\n{} Docker API not exposed or requires auth.",
            "[-]".green().bold()
        );
    }
    Ok(())
...
pub async fn kubelet(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let endpoints = [
        ...
        (
            "Exec in pod",
            "/exec/default/pod/container?command=id&output=1",
        ),
    ...
    for (name, path) in &endpoints {
            ...
            .await
        {
            Ok(resp) => {
                ...
                println!("  {} {:25} status={} {}", "*".cyan(), name, status, tag);
                if accessible {
                    exposed.push((name.to_string(), body.chars().take(200).collect::<String>()));
                }
            }
            Err(_) => {
                println!("  {} {:25} error", "*".red(), name);
    ...
    if !exposed.is_empty() {
        println!(
            "\n{} Kubelet API exposed! {} endpoints accessible.",
            "[!]".red().bold(),
            exposed.len()
        );
        for (name, body) in &exposed {
            println!(
                "  {} {} — {}",
                "*".red(),
                name,
        ...
        println!(
            "\n{} Kubelet API requires auth or not exposed.",
            "[-]".green().bold()
        );
    }
    Ok(())
    ...
    for (cap, desc) in &caps {
            ...
            .await
        {
            Ok(resp) => {
                ...
                let has_cap =
                    resp_body.contains("true") || resp_body.contains("granted") || status == 200;
                let tag = if has_cap {
                    "GRANTED".red().bold().to_string()
                } else {
                    "denied".green().to_string()
                };
                println!("  {} {:25} {} — {}", "*".cyan(), cap, tag, desc);
            }
            Err(_) => {
                println!("  {} {:25} error", "*".red(), cap);
            }
        }
    }

    println!(
        "\n{} Dangerous capabilities can enable container escape.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn mount(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for (name, container_path, host_path) in &mounts {
            ...
            .await
        {
            Ok(resp) => {
                let status = resp.status().as_u16();
                let resp_body = resp.text().await.unwrap_or_default();
                let readable = resp_body.contains("passwd")
                    || resp_body.contains("shadow")
                    || resp_body.contains("root")
                    || resp_body.contains("docker")
                    || resp_body.contains("containerd")
                    || !resp_body.is_empty();
                let tag = if readable && status == 200 {
                    "READABLE".red().bold().to_string()
                } else {
                    "not mounted".green().to_string()
                };
    ...
    if !accessible.is_empty() {
        println!(
            "\n{} {} host mount(s) accessible — escape possible!",
            "[!]".red().bold(),
            accessible.len()
        );
    } else {
