...
pub async fn leak(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let has_webrtc = body.contains("RTCPeerConnection")
        || body.contains("webkitRTCPeerConnection")
        || body.contains("mozRTCPeerConnection")
        || body.contains("createDataChannel")
        || body.contains("onicecandidate")
        || body.contains("ICE");

    if has_webrtc {
        println!("  {} WebRTC API detected in page", "[!]".red().bold());
        println!(
            "  {} The page uses RTCPeerConnection — potential IP leak",
            "*".yellow()
        );
        println!(
            "  {} Local and public IPs may be exposed bypassing VPN/proxy",
            "*".yellow()
        );
    } else {
    ...
    if let Some(re) = turn_pattern {
        ...
        if !turns.is_empty() {
            ...
            for t in &turns {
                println!("    {} {} (may contain credentials)", "*".red(), t);
            }
        }
    }

    println!(
        "\n{} WebRTC can leak real IP even behind VPN/proxy.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn stun(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if let Some(re) = stun_re {
        for m in re.find_iter(&body) {
            stun_servers.push(m.as_str().to_string());
        }
    }
    ...
    if let Some(re) = turn_re {
        for m in re.find_iter(&body) {
            turn_servers.push(m.as_str().to_string());
        }
    }
    ...
    if stun_servers.is_empty() && turn_servers.is_empty() {
        ...
        println!(
            "  {} No STUN/TURN found in page. Testing common servers:",
            "[*]".cyan().bold()
        );
        for s in &common_stun {
            println!("    {} {}", "*".cyan(), s);
        }
    ...
    println!(
        "\n  {} STUN servers (can reveal public IP):",
        "[*]".cyan().bold()
    );
    ...
    if !turn_servers.is_empty() {
        println!(
            "\n  {} TURN servers (relay — can access internal network):",
            "[*]".cyan().bold()
        );
        for t in &turn_servers {
            let has_creds = t.contains("credential")
                || t.contains("username")
                || t.contains(":") && t.split(':').count() > 3;
            let tag = if has_creds {
                "HAS CREDS".red().bold().to_string()
            } else {
                "no creds".to_string()
            };
            println!("    {} {} {}", "*".red(), t, tag);
    ...
    println!(
        "\n{} STUN can be used for internal network discovery via ICE candidates.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn relay(url: &str, timeout: u64) -> anyhow::Result<()> {
    println!(
        "{} WebRTC Relay for Internal Network Discovery",
        "[*]".cyan().bold()
    );
    println!("{}", "=".repeat(60).cyan());
    println!("{} URL: {}", "[*]".cyan().bold(), url);
    ...
    if !has_webrtc {
        println!(
            "  {} No WebRTC detected — cannot relay.",
            "[-]".yellow().bold()
        );
        return Ok(());
    }
    ...
    println!(
        "  {} WebRTC detected — checking for internal IP leak vectors:",
        "[*]".cyan().bold()
    );
    for (range, prefix) in &internal_ranges {
        if body.contains(prefix) {
            println!(
                "    {} [LEAK] Internal IP range {} detected in page!",
                "[!]".red().bold(),
                range
            );
        } else {
    ...
    for p in &ice_patterns {
        if body.contains(p) {
            found_patterns.push(p.to_string());
        }
    }
    ...
    println!(
        "\n{} WebRTC can discover internal network topology via host ICE candidates.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn fingerprint(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if found.is_empty() {
        println!(
            "  {} No WebRTC fingerprinting vectors found.",
            "[-]".green().bold()
        );
    } else {
        println!(
            "\n{} {} WebRTC fingerprinting vector(s) found.",
            "[!]".red().bold(),
            found.len()
        );
        println!(
            "{} These can be used for browser fingerprinting and tracking.",
            "[*]".cyan().bold()
        );
    }
    Ok(())
