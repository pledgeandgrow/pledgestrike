...
pub async fn cipher0(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    {
        println!(
            "  {} Cipher 0 authentication bypass successful!",
            "[!]".red().bold()
        );
        println!(
            "  {} Password hashes can be extracted without authentication.",
            "[!]".red().bold()
        );
    } else {
    ...
    let hash_body =
        serde_json::json!({"action": "ipmi_rakp", "host": url, "username": "admin", "cipher": 0});
...
pub async fn bmc(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for (name, check) in &checks {
        ...
        match client.post(url).json(&body).send().await {
            Ok(r) => {
                let text = r.text().await.unwrap_or_default();
                let vulnerable = text.contains("vulnerable")
                    || text.contains("found")
