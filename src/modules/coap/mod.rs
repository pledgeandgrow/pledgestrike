...
pub async fn discover(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if status == 200 && !body.is_empty() {
        println!("  {} CoAP resource discovery successful!", "[+]".green().bold());
        println!("  {} Available resources:", "[*]".cyan().bold());
        for line in body.lines().take(30) {
            println!("    {}", line);
        }
    } else {
        println!("  {} .well-known/core not available, trying common paths:", "[-]".dimmed(), );
    }

    let common_resources = ["/", "/sensors", "/actuators", "/config", "/status", "/firmware", "/admin", "/debug", "/system", "/metrics", "/led", "/button", "/temperature", "/humidity", "/light"];
    ...
    for res in &common_resources {
        ...
        match client.get(&target).send().await {
            Ok(r) => {
                ...
                if s == 200 && !t.is_empty() {
                    println!("    {} {:20} — {} bytes: {}", "[+]".green().bold(), res, t.len(), t.chars().take(50).collect::<String>());
                } else if s != 404 {
        ...
        match client.request(m, url).body(*payload).send().await {
            Ok(r) => {
                ...
                let tag = if amplification > 10.0 {
                    format!("{} bytes, amp={:.1}x — DANGEROUS", resp_size, amplification).red().bold().to_string()
                } else if amplification > 1.0 {
                    format!("{} bytes, amp={:.1}x", resp_size, amplification).yellow().to_string()
                } else {
...
pub async fn access(url: &str, timeout: u64) -> anyhow::Result<()> {
        ...
        match client.request(m, &target).body("value=test").send().await {
            Ok(r) => {
                ...
                let tag = if status == 200 || status == 201 || status == 204 {
                    format!("status={} — UNAUTHORIZED ACCESS", status).red().bold().to_string()
                } else {
    ...
    let sensitive_resources = ["/admin", "/firmware", "/config", "/system/reset", "/system/factory", "/keys", "/certs", "/auth"];
    ...
    for res in &sensitive_resources {
        ...
        match client.get(&target).send().await {
            Ok(r) => {
                ...
                let t = r.text().await.unwrap_or_default();
                if s == 200 && !t.is_empty() {
                    println!("    {} {:20} — ACCESSIBLE: {}", "[!]".red().bold(), res, t.chars().take(50).collect::<String>());
                }
            }
            Err(_) => {}
        }
...
pub async fn cache(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let client = build_client(timeout);
    let attacks = [
        ("Response forgery", "Inject crafted CoAP response with Max-Age to poison cache"),
        ("Observe notification abuse", "Register as observer and send fake notifications to other clients"),
        ("Proxy cache poisoning", "Send request through CoAP proxy with crafted ETag to poison cached response"),
        ("Max-Age manipulation", "Set Max-Age to very high value to persist poisoned cache entry"),
        ("ETag collision", "Craft request with known ETag to force cache hit with wrong content"),
    ];

    for (name, desc) in &attacks {
        let body = serde_json::json!({"action": "coap_attack", "host": url, "type": name, "desc": desc});
        match client.post(url).json(&body).send().await {
            Ok(r) => {
                let status = r.status().as_u16();
                let text = r.text().await.unwrap_or_default();
                let success = text.contains("poisoned") || text.contains("injected") || status == 200;
                let tag = if success { "CACHE POISONED".red().bold().to_string() } else { format!("status={}", status) };
