...
const SECRET_PATTERNS: &[(&str, &str)] = &[
    ...
    ("MySQL URI", "mysql://[^\\s]+"),
    ("Redis URI", "redis://[^\\s]+"),
    ("Generic Secret", "secret[\"']?\\s*[:=]\\s*[\"']?[0-9A-Za-z\\-_]{16,}"),
    ("Password", "password[\"']?\\s*[:=]\\s*[\"']?[^\"'\\s]{8,}"),
    ("Client Secret", "client[_-]secret[\"']?\\s*[:=]\\s*[\"']?[0-9A-Za-z\\-_]{20,}"),
...
pub async fn js(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    println!("{} {} secret patterns", "[*]".cyan().bold(), SECRET_PATTERNS.len());
    ...
    let js_urls: Vec<&str> = body.matches("src=\"").map(|m| {
        let start = m.as_ptr() as usize - body.as_ptr() as usize + 5;
        let end = body[start..].find('"').map(|e| start + e).unwrap_or(start);
        &body[start..end]
    }).filter(|s| s.ends_with(".js")).collect();

    println!("{} Found {} JS file(s) in page", "[*]".cyan().bold(), js_urls.len());
    ...
    for js_url in &js_urls {
        let full_url = if js_url.starts_with("http") { js_url.to_string() } else if js_url.starts_with("/") { format!("{}{}", url.trim_end_matches('/'), js_url) } else { format!("{}/{}", url.trim_end_matches('/'), js_url) };
    ...
    if all_secrets.is_empty() {
        ...
        println!("\n{} {} secret(s) found:", "[!]".red().bold(), all_secrets.len());
        for (name, value, source) in &all_secrets {
            println!("  {} {:20} in {} — {}...", "*".red(), name, source, value.chars().take(60).collect::<String>());
        }
    }
    Ok(())
}
...
pub async fn repo(url: &str, timeout: u64, token: Option<&str>) -> anyhow::Result<()> {
    ...
    let secret_files = [".env", "config.json", "config.yml", "settings.py", "application.properties", "secrets.yaml", "credentials.json"];
...
pub async fn response(url: &str, timeout: u64, token: Option<&str>) -> anyhow::Result<()> {
    ...
    if all_secrets.is_empty() {
        println!("\n{} No secrets found in API response.", "[-]".green().bold());
    } else {
        println!("\n{} {} secret(s) found:", "[!]".red().bold(), all_secrets.len());
        for (name, value, source) in &all_secrets {
            println!("  {} {:20} in {} — {}...", "*".red(), name, source, value.chars().take(60).collect::<String>());
...
pub async fn docker(url: &str, timeout: u64, token: Option<&str>) -> anyhow::Result<()> {
    ...
    if all_secrets.is_empty() {
        println!("\n{} No secrets found in Docker layers.", "[-]".green().bold());
    } else {
        println!("\n{} {} secret(s) found in Docker layers:", "[!]".red().bold(), all_secrets.len());
        for (name, value, source) in &all_secrets {
            println!("  {} {:20} in {} — {}...", "*".red(), name, source, value.chars().take(60).collect::<String>());
        }
    }
