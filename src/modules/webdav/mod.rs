...
pub async fn methods(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let allowed: Vec<&str> = webdav_methods
        .iter()
...
<D:propfind xmlns:D="DAV:">
  <D:allprop/>
    ...
    if status == 207 {
        println!(
            "  {} PROPFIND successful (207 Multi-Status)",
            "[+]".green().bold()
        );
        ...
        if !snippet.is_empty() {
            println!("  {} Response: {}", "*".dimmed(), snippet);
        }
    }
    Ok(())
}
...
pub async fn upload(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let test_files = [
        (
            "ps_test.html",
            "text/html",
            "<html><body>PledgeStrike test</body></html>",
        ),
        ("ps_test.txt", "text/plain", "PledgeStrike test file"),
        (
            "ps_test.aspx",
            "text/html",
            "<%@ Page Language=\"C#\" %><% Response.Write(\"PS\"); %>",
        ),
    ...
    for (filename, content_type, content) in &test_files {
            ...
            .await
        {
            Ok(resp) => {
                ...
                if uploaded {
                    ...
                    if let Ok(vresp) = verify {
                        ...
                        let vtag = if accessible {
                            "EXECUTED".red().bold().to_string()
                        } else if vstatus == 200 {
                            "accessible".to_string()
                        } else {
                            format!("status={}", vstatus)
                        };
            ...
            Err(_) => {
                println!("  {} {:25} error", "*".red(), filename);
            }
        }
    }

    println!(
        "\n{} If upload succeeds and file is accessible, webshell deployment is possible.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn copy(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "\n{} COPY/MOVE can overwrite config files or move webshells to webroot.",
        "[*]".cyan().bold()
    );
    Ok(())
}
