...
pub async fn fixation(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for sid in &fixated_ids {
        let cookie_val = format!(
            "PHPSESSID={}; JSESSIONID={}; session={}; sid={}",
            sid, sid, sid, sid
        );
        match client.get(url).header("Cookie", &cookie_val).send().await {
            Ok(r) => {
                ...
                if resp_cookie.contains(sid) {
                    println!(
                        "  {} Session {:25} — ACCEPTED (no rotation)",
                        "[!]".red().bold(),
                        sid
                    );
                } else if !resp_cookie.is_empty() {
                    println!(
                        "  {} Session {:25} — rotated (secure)",
                        "[+]".green().bold(),
                        sid
                    );
                } else {
...
pub async fn predict(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for i in 0..5 {
        if let Ok(r) = client.get(url).send().await
            && let Some(cookie) = r.headers().get("set-cookie")
        {
            let cookie_str = cookie.to_str().unwrap_or("");
            if let Some(start) = cookie_str.find('=') {
                let end = cookie_str[start + 1..]
                    .find(';')
                    .map(|e| start + 1 + e)
    ...
    if tokens.len() >= 2 {
        ...
        if all_same {
            println!(
                "\n  {} All tokens identical — no rotation!",
                "[!]".red().bold()
            );
        }
        ...
        if all_same_len && len < 16 {
            println!(
                "  {} Short token length ({}) — predictable",
                "[!]".red().bold(),
                len
            );
        }
...
pub async fn puzzle(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for i in 0..10 {
        if let Ok(r) = client.get(url).send().await {
            ...
            println!(
                ...
                "*".cyan(),
                i,
                &set_cookie[..set_cookie.len().min(30)]
            );
        }
    }

    let unique_cookies: Vec<&String> = sessions.iter().map(|(c, _)| c).collect();
    let unique_count = unique_cookies
        .iter()
        .collect::<std::collections::HashSet<_>>()
        .len();
    println!(
        "\n  {} {} unique sessions out of {} requests",
        "[*]".cyan().bold(),
        unique_count,
        sessions.len()
    );
    if unique_count < 3 {
        println!(
            "  {} Low session diversity — puzzle attack viable",
            "[!]".red().bold()
        );
    }
