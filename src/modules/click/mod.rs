...
pub async fn frame(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if xfo.is_empty() && !csp.contains("frame-ancestors") {
        ...
        if xfo.to_lowercase().contains("deny") {
            println!(
                "  {} XFO: DENY — framing blocked by most browsers.",
                "[-]".green().bold()
            );
        } else if xfo.to_lowercase().contains("sameorigin") {
            println!(
                "  {} XFO: SAMEORIGIN — cross-origin framing blocked.",
                "[-]".green().bold()
            );
            println!(
                "  {} Bypass: try ancestor chain, iframe within same origin, or open redirect.",
                "[*]".yellow().bold()
            );
        } else if xfo.to_lowercase().contains("allow-all") {
            println!(
                "  {} XFO: ALLOW-ALL — framing allowed from any origin!",
                "[!]".red().bold()
            );
        }
    }
    ...
    if csp.contains("frame-ancestors") {
        ...
        {
            println!(
                "  {} CSP frame-ancestors may allow framing from external origins.",
                "[!]".red().bold()
            );
        }
    }
...
pub async fn overlay(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if !found.is_empty() {
        println!(
            "  {} Overlay-related CSS patterns found:",
            "[+]".yellow().bold()
        );
        for f in &found {
            println!("    {} {}", "*".cyan(), f);
    ...
    let poc_overlays = [
        (
            ...
            "opacity:0 button positioned over decoy",
        ),
        (
            "Full-page iframe overlay",
            "100% width/height transparent iframe over page",
        ),
        (
            "Cursor offset trick",
            "cursor: none with fake cursor at offset position",
        ),
        (
            "Drag-and-drop capture",
            "hidden draggable element capturing mouse events",
        ),
    ];
...
pub async fn pointer(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if !found.is_empty() {
        println!(
            "  {} Pointer/touch event handlers detected:",
            "[+]".yellow().bold()
        );
        for f in &found {
            println!("    {} {}", "*".cyan(), f);
    ...
    let techniques = [
        (
            ...
            "Transparent iframe captures touchstart/touchend on mobile",
        ),
        (
            "Double-click timing",
            "First click activates iframe, second click hits target",
        ),
        (
            "Pointer capture abuse",
            "setPointerCapture() to redirect pointer events",
        ),
        (
            "Drag event hijacking",
            "dragstart/dragend events to trigger actions in hidden iframe",
        ),
    ];
...
pub async fn cursor(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if !found.is_empty() {
        println!(
            "  {} Custom cursor patterns detected:",
            "[+]".yellow().bold()
        );
        for f in &found {
            println!("    {} {}", "*".cyan(), f);
    ...
    let techniques = [
        (
            "Invisible cursor",
            "cursor:none + fake cursor at offset to misdirect clicks",
        ),
        (
            "SVG cursor overlay",
            "Custom SVG cursor that appears as normal but clicks at offset",
        ),
        (
            "Animated cursor",
            "Animated cursor that moves independently of actual pointer",
        ),
        (
            "Cursor lag exploit",
            "CSS transition on cursor position to create lag-based misclicks",
        ),
    ];
