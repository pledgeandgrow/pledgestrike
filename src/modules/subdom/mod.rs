...
pub async fn brute(domain: &str, timeout: u64, _wordlist: Option<&str>) -> anyhow::Result<()> {
        ...
        if let Ok(resp) = client.get(ct_url).send().await {
            ...
            if let Ok(entries) = serde_json::from_str::<serde_json::Value>(&body) {
                        ...
                        if let Some(names) = entry.get("name_value").and_then(|n| n.as_str()) {
                            for name in names.split('\n') {
                                ...
                                if name.ends_with(domain) {
                                    all_subs.insert(name.to_string());
                                }
                            }
                        }
                    }
                ...
                for line in body.lines() {
                    let line = line.trim();
                    if line.ends_with(domain) {
                        all_subs.insert(line.to_string());
                    }
                }
            }
        }
...
pub async fn passive(domain: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let sources = [
        ...
        (
            "RapidDNS",
            format!("https://rapiddns.io/subdomain/{}?full=1", domain),
        ),
    ];
        ...
        match client.get(url).send().await {
            Ok(resp) => {
                ...
                for line in body.split(['\n', ',', '"', ' ']) {
                    ...
                    if line.ends_with(domain) && line != domain {
                        all_subs.insert(line.to_string());
                    }
                }
    ...
    println!(
        "\n{} {} total unique subdomain(s) from passive sources:",
        "[*]".cyan().bold(),
        all_subs.len()
    );
    for sub in &all_subs {
...
pub async fn permutate(domain: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let prefixes = [
        "dev", "stg", "staging", "prod", "test", "qa", "uat", "pre", "post", "v1", "v2", "new",
        "old", "beta", "alpha", "internal", "ext", "int",
    ];
    let separators = ["-", ".", "_", ""];
    let base_words = [
        "api", "app", "web", "admin", "portal", "mail", "git", "ci", "vpn", "auth", "sso",
        "grafana", "k8s", "docker", "cloud", "shop", "store", "mobile", "cdn",
    ];
    ...
    println!(
        "{} {} permutations generated, resolving...",
        "[*]".cyan().bold(),
        permutations.len()
    );
