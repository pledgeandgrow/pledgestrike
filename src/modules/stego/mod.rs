...
const STEGO_TECHNIQUES: &[(&str, &str)] = &[
    ...
    (
        "LSB — PNG RGBA",
        "Least Significant Bit in PNG RGBA channels",
    ),
    ...
    (
        ...
        "Data appended after image end marker",
    ),
...
const STEGO_INDICATORS: &[(&str, &str)] = &[
    (
        "File size anomaly",
        "Image larger than expected for dimensions",
    ),
...
pub async fn detect(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "{} {} image types, {} stego techniques, {} indicators",
        "[*]".cyan().bold(),
        IMAGE_EXTENSIONS.len(),
        STEGO_TECHNIQUES.len(),
        STEGO_INDICATORS.len()
    );
    ...
    println!(
        "\n{} [2/3] Steganography analysis on {} images...",
        "[*]".cyan().bold(),
        images.len()
    );
    ...
    for (path, size, content_type, data) in &images {
        ...
        if is_png {
            ...
            if trailing > 0 {
                println!(
                    "    {} [HIGH] Trailing data after IEND: {} bytes",
                    "!".red().bold(),
                    trailing
                );
            ...
            for (chunk_type, chunk_size) in &chunks {
                ...
                if chunk_type == "Unknown" {
                    println!(
                        "    {} [MED] Unknown chunk: {} bytes",
                        "!".yellow(),
                        chunk_size
                    );
                    findings.push(format!(
                        "{}: unknown PNG chunk ({} bytes)",
                        path, chunk_size
                    ));
                }
            }
        ...
        if is_jpeg {
            ...
            if trailing > 0 {
                println!(
                    "    {} [HIGH] Trailing data after FFD9: {} bytes",
                    "!".red().bold(),
                    trailing
                );
                findings.push(format!(
            ...
            if exif_size > 1000 {
                println!(
                    "    {} [MED] Large EXIF segment: {} bytes",
                    "!".yellow(),
                    exif_size
                );
                findings.push(format!(
                    "{}: large EXIF segment ({} bytes)",
                    path, exif_size
                ));
            }
            ...
            if comment_size > 100 {
                println!(
                    "    {} [MED] Large comment segment: {} bytes",
                    "!".yellow(),
                    comment_size
                );
                findings.push(format!(
                    "{}: large JPEG comment ({} bytes)",
                    path, comment_size
                ));
            }
        }
        ...
        if is_gif {
            ...
            if trailing > 0 {
                println!(
                    "    {} [HIGH] Trailing data after GIF trailer: {} bytes",
                    "!".red().bold(),
                    trailing
                );
                findings.push(format!(
                    "{}: trailing data after GIF trailer ({} bytes)",
                    path, trailing
                ));
            }
        }
        ...
        if lsb_score > 0.7 {
            println!(
                "    {} [HIGH] LSB anomaly score: {:.2} — possible LSB steganography",
                "!".red().bold(),
                lsb_score
            );
            ...
            println!(
                "    {} [MED] LSB anomaly score: {:.2}",
                "?".yellow(),
                lsb_score
            );
        } else {
            println!(
                "    {} [OK] LSB analysis: {:.2} (normal)",
                ".".green(),
                lsb_score
            );
        }
        ...
        if entropy > 7.5 {
            println!(
                "    {} [MED] High entropy: {:.2} — possible encrypted data",
                "?".yellow(),
                entropy
            );
        } else {
    ...
    println!(
        "\n{} [3/3] Steganography technique reference...",
        "[*]".cyan().bold()
    );
    ...
    if !findings.is_empty() {
        println!(
            "{} [HIGH] Steganographic anomalies detected:",
            "[!]".red().bold()
        );
        for f in &findings {
            println!("    {} {}", ">".red(), f);
        ...
        println!(
            "{} No steganographic anomalies detected in {} images",
            "[-]".green().bold(),
            images.len()
        );
    }
