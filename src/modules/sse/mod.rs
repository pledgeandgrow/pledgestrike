...
pub async fn inject(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if !is_sse {
        println!(
            "  {} Endpoint is not SSE (Content-Type: {}).",
            "[-]".yellow().bold(),
            content_type
        );
        return Ok(());
    ...
    println!(
        "  {} SSE endpoint confirmed (Content-Type: {})",
        "[+]".green().bold(),
        content_type
    );
    ...
    println!(
        "\n  {} Injection payloads (inject into SSE stream data):",
        "[*]".cyan().bold()
    );
    ...
    println!(
        "\n{} If SSE data is reflected without sanitization, XSS is possible.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn exhaust(url: &str, count: u32, timeout: u64) -> anyhow::Result<()> {
    ...
    if ok > 50 {
        println!(
            "{} Target allows many persistent SSE connections — DoS vector.",
            "[!]".red().bold()
        );
    }
    Ok(())
...
pub async fn exfil(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let sensitive_patterns = [
        "token", "secret", "password", "key", "auth", "session", "user", "email", "ssn", "credit",
    ];
    ...
    if leaks.is_empty() {
        println!(
            "  {} No sensitive data detected in SSE stream.",
            "[-]".green().bold()
        );
    } else {
        println!(
            "\n  {} Potential data leaks in SSE stream:",
            "[!]".red().bold()
        );
    ...
    println!(
        "\n{} SSE provides a persistent covert channel for data exfiltration.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn replay(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "\n{} Last-Event-ID can trigger event replay or injection if not validated.",
        "[*]".cyan().bold()
    );
    Ok(())
}
