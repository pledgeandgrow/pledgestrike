...
pub async fn axfr(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "\n  {} Testing common nameservers for AXFR:",
        "[*]".cyan().bold()
    );
    for ns in &nameservers {
        let ns_domain = format!(
            "{}.{}",
            ns,
        ...
        let req_body =
            serde_json::json!({"action": "dns_axfr", "domain": url, "server": ns_domain});
    ...
    if status == 200 && (text.contains("XFR") || text.contains("transfer")) {
        println!(
            "\n{} AXFR response indicates zone transfer was permitted!",
            "[!]".red().bold()
        );
    } else {
        println!(
            "\n{} Zone transfer not permitted on tested servers.",
            "[-]".green().bold()
        );
    }

    println!(
        "\n{} Note: Full AXFR testing requires direct DNS protocol access (port 53).",
        "[*]".yellow().bold()
    );
    println!(
        "{} Use dig @server domain AXFR for direct testing.",
        "[*]".yellow().bold()
    );
    Ok(())
}
...
pub async fn records(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "\n{} Also checking common subdomain records:",
        "[*]".cyan().bold()
    );
    let common_subs = [
...
pub async fn nsec(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if status == 200 && (text.contains("NSEC") || text.contains("NSEC3")) {
        println!(
            "  {} DNSSEC is enabled — NSEC records found!",
            "[+]".green().bold()
        );
        println!(
            "  {} NSEC walking may enumerate hidden subdomains.",
            "[*]".cyan().bold()
        );
        ...
        for i in 0..20 {
            let walk_body =
                serde_json::json!({"action": "dns_query", "domain": current, "type": "NSEC"});
        ...
        println!(
            "  {} No NSEC/NSEC3 records found — zone is not DNSSEC-signed or NSEC3 used.",
            "[-]".dimmed()
        );
    }

    let nsec3_body =
        serde_json::json!({"action": "dns_query", "domain": domain, "type": "NSEC3PARAM"});
    ...
    if text3.contains("NSEC3PARAM") {
        println!(
            "\n  {} NSEC3PARAM found — NSEC3 walking is harder but possible with tools like nsec3walker.",
            "[*]".yellow().bold()
        );
    }
...
pub async fn snoop(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "  {} Querying resolver for cached domains (non-recursive):",
        "[*]".cyan().bold()
    );
    for domain in &test_domains {
        let body =
            serde_json::json!({"action": "dns_query", "domain": domain, "type": "A", "rd": false});
        match client.post(url).json(&body).send().await {
            Ok(resp) => {
                let status = resp.status().as_u16();
                let text = resp.text().await.unwrap_or_default();
    ...
    println!(
        "\n{} Cached domains indicate recent DNS lookups by users of this resolver.",
        "[*]".yellow().bold()
    );
    Ok(())
}
