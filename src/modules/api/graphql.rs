...
pub async fn graphql(
    ...
    match resp {
        Ok(r) => {
            ...
            if status == 200 && (body.contains("__typename") || body.contains("data")) {
                println!(
                    "{} GraphQL endpoint confirmed! (HTTP {})",
                    "[+]".green().bold(),
                    status
                );
            } else if body.to_lowercase().contains("graphql") {
                println!(
                    "{} Likely GraphQL endpoint (HTTP {})",
                    "[+]".green().bold(),
                    status
                );
            } else {
                println!(
                    "{} Endpoint responded (HTTP {}) — may not be GraphQL",
                    "[?]".yellow().bold(),
                    status
                );
                ...
                match serde_json::from_str::<Value>(&body) {
                    Ok(v) => {
                        if let Some(schema) = v.get("data").and_then(|d| d.get("__schema")) {
                            println!(
                                "{} Introspection ENABLED — schema leaked!",
                                "[+]".green().bold().blink()
                            );
                            ...
                            {
                                let custom: Vec<&Value> = types_arr
                                    .iter()
                                    .filter(|t| {
                                        let kind =
                            ...
                            println!(
                                "{} Introspection disabled — errors returned:",
                                "[-]".red().bold()
                            );
    ...
    if suggest {
        ...
        for field in &wordlist {
            ...
            if let Ok(r) = client.post(endpoint_url).json(&query).send().await {
                let body = r.text().await.unwrap_or_default();
    ...
    if let Ok(r) = client.post(endpoint_url).json(&batch).send().await {
        let status = r.status().as_u16();
        let body = r.text().await.unwrap_or_default();
        if status == 200 && body.starts_with('[') {
            println!(
                "{} Batch queries ENABLED — potential DoS vector",
                "[!]".yellow().bold()
            );
        } else {
    ...
    if let Ok(r) = client.post(endpoint_url).json(&deep_query).send().await {
        let status = r.status().as_u16();
        let body = r.text().await.unwrap_or_default();
        if status == 200 && !body.contains("depth") && !body.contains("Depth") {
            println!(
                "{} No query depth limit — potential DoS via deep nesting",
                "[!]".yellow().bold()
            );
