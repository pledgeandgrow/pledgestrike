...
pub async fn http(
    ...
    let users: Vec<String> = std::fs::read_to_string(users_file)?
        ...
        .filter(|s| !s.is_empty())
        .collect();
    let passwords: Vec<String> = std::fs::read_to_string(pass_file)?
        .lines()
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty())
        .collect();
    ...
    let combos: Vec<(String, String)> = users
        .iter()
        ...
        handles.push(tokio::spawn(async move {
            loop {
                ...
                if n.is_multiple_of(100) {
                    print!("\r{} Attempts: {}/{}", "*".dimmed(), n, combos.len());
                }
...
pub async fn ssh(
    ...
    for user in &users {
        for pass in &passwords {
            ...
            if attempts.is_multiple_of(50) {
                print!("\r{} Attempts: {}", "*".dimmed(), attempts);
            }
    ...
    println!(
        "\n\n{} Results: {} valid credential(s) after {} attempts",
        "[*]".cyan().bold(),
        found.len(),
        attempts
    );
    ...
    println!(
        "{} Note: SSH brute force requires ssh2/russh crate for full implementation.",
        "[*]".cyan().bold()
    );
    Ok(())
}
    ...
    for user in &users {
        for pass in &passwords {
            ...
            if let Ok(mut stream) = tokio::net::TcpStream::connect((host, port)).await {
                ...
                if !user_resp.contains("331") && !user_resp.contains("230") {
                    continue;
                }
