...
pub async fn analyze(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if !is_wasm {
        ...
        if body_str.contains("application/wasm") || body_str.contains(".wasm") || body_str.contains("WebAssembly") {
            println!("  {} Page references WASM but didn't fetch a .wasm file directly.", "*".yellow());
            let wasm_re = regex::Regex::new(r#"(?:src|href)=["']([^"']+\.wasm)["']"#).ok();
            if let Some(re) = wasm_re {
                let matches: Vec<_> = re.find_iter(&body_str).map(|m| m.as_str().to_string()).collect();
                if !matches.is_empty() {
                    println!("  {} WASM files referenced:", "[*]".cyan().bold());
                    for m in &matches { println!("    {} {}", "*".cyan(), m); }
                }
            }
        } else {
    ...
    println!("  {} Valid WASM binary detected ({} bytes)", "[+]".green().bold(), body.len());
    ...
    while i < body.len() {
        ...
        if section_end > body.len() { break; }

        match section_id {
            2 => {
                let mut j = i;
                let (count, c) = read_leb128(&body, j); j += c;
                for _ in 0..count {
                    if j >= section_end { break; }
                    let (mod_len, c) = read_leb128(&body, j); j += c;
                    let mod_name = String::from_utf8_lossy(&body[j..j+mod_len as usize]).to_string();
                    j += mod_len as usize;
                    let (name_len, c) = read_leb128(&body, j); j += c;
                    let name = String::from_utf8_lossy(&body[j..j+name_len as usize]).to_string();
                    j += name_len as usize;
                    j += 1;
                    let (_, c) = read_leb128(&body, j); j += c;
    ...
    if !imports.is_empty() {
        ...
        for imp in imports.iter().take(20) { println!("    {} {}", "*".cyan(), imp); }
        if imports.len() > 20 { println!("    ... and {} more", imports.len() - 20); }
    }
    if !exports.is_empty() {
        println!("\n  {} Exports ({}):", "[*]".cyan().bold(), exports.len());
        for exp in exports.iter().take(20) { println!("    {} {}", "*".cyan(), exp); }
        if exports.len() > 20 { println!("    ... and {} more", exports.len() - 20); }
    }

    let dangerous_imports = imports.iter().filter(|i| i.contains("fd_write") || i.contains("abort") || i.contains("env") || i.contains("memory") || i.contains("eval")).count();
    if dangerous_imports > 0 {
        println!("\n{} {} potentially dangerous import(s) found.", "[!]".red().bold(), dangerous_imports);
    }
    Ok(())
}
...
pub async fn memory(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let patterns = [
        ("instance.exports.memory", "Memory export — JS can read/write WASM memory"),
    ...
    if found.is_empty() {
        println!("  {} No WASM memory access patterns found.", "[-]".green().bold());
    } else {
        println!("\n{} {} memory access pattern(s) — secrets in WASM memory may be readable.", "[!]".red().bold(), found.len());
    }
    Ok(())
}
...
pub async fn import(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let import_patterns = [
        ("env.fd_write", "File descriptor write — can intercept output"),
        ...
        ("env.emscripten_run_script", "Script execution — RCE via emscripten"),
    ...
    if found.is_empty() {
        println!("  {} No dangerous import patterns found.", "[-]".green().bold());
    } else {
        println!("\n{} {} dangerous import(s) — hook these to intercept WASM behavior.", "[!]".red().bold(), found.len());
    }
    Ok(())
}
...
pub async fn reverse(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    while i < body.len() {
        ...
        i += consumed;
        let section_names = ["Custom", "Type", "Import", "Function", "Table", "Memory", "Global", "Export", "Start", "Element", "Code", "Data", "DataCount"];
        let name = section_names.get(section_id as usize).map(|s| *s).unwrap_or("Unknown");
    ...
    if let Some((_, size)) = code_section {
        println!("\n  {} Code section: {} bytes — use wasm2wat or wasm-decompile for full decompilation.", "[*]".cyan().bold(), size);
    }

    println!("\n  {} Tools: wasm2wat, wasm-decompile, wasm-objdump, ghidra-wasm-plugin", "[*]".cyan().bold());
    Ok(())
}
