...
pub async fn enum_smb(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if status == 200 {
        ...
        println!("  {} SMB enumeration failed (status={})", "[-]".dimmed(), status);
    }

    let common_shares = ["C$", "D$", "ADMIN$", "IPC$", "NETLOGON", "SYSVOL", "print$", "Users", "Public", "Shared", "Backup", "Data"];
    ...
    for share in &common_shares {
        ...
        match client.post(url).json(&share_body).send().await {
            Ok(r) => {
                ...
                let accessible = t.contains("read") || t.contains("write") || t.contains("access") || t.contains("200");
                let tag = if accessible { "ACCESSIBLE".red().bold().to_string() } else { "no access".dimmed().to_string() };
...
pub async fn eternal(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    if text.contains("vulnerable") || text.contains("VULNERABLE") || (status == 200 && text.contains("ms17-010")) {
        println!("  {} Target is VULNERABLE to EternalBlue (MS17-010)!", "[!]".red().bold());
        println!("  {} This allows remote code execution via SMBv1.", "[!]".red().bold());
    } else if text.contains("safe") || text.contains("patched") || text.contains("not vulnerable") {
        println!("  {} Target is patched against MS17-010.", "[-]".green().bold());
    } else {
...
pub async fn relay(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    for (name, check) in &checks {
        ...
        match client.post(url).json(&body).send().await {
            Ok(r) => {
                ...
                let tag = if required { "required/enforced".green().bold().to_string() }
                    else if enabled { "enabled (not enforced)".yellow().to_string() }
                    else if text.contains("disabled") || text.contains("false") { "disabled".red().bold().to_string() }
                    else { text.chars().take(30).collect() };
    ...
    println!("\n{} SMB signing not required = relay attack possible.", "[*]".yellow().bold());
    Ok(())
}
