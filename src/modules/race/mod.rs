...
pub async fn race(
    ...
    if success > 1 {
        println!(
            "\n{} [HIGH] {} successful responses — possible race condition!",
            "[!]".red().bold(),
            success
        );
    } else if conflicts > 0 {
        println!(
            "\n{} Server detected conflicts (409). Race condition protection may exist.",
            "[-]".yellow().bold()
        );
    } else if rate_limited > 0 {
        println!(
            "\n{} Rate limited (429). Server has rate limiting protection.",
            "[-]".yellow().bold()
        );
    } else {
...
pub async fn toctou(url: &str, token: Option<&str>, timeout: u64) -> anyhow::Result<()> {
    println!(
    ...
    println!(
        "{} Step 1: Baseline request (check state)...",
        "[*]".cyan().bold()
    );
    ...
    let (status1, body1) = match resp1 {
        ...
        Err(_) => {
            println!("{} Baseline request failed.", "[-]".red().bold());
            return Ok(());
        }
    };
    println!(
    ...
    println!(
        "{} Step 2: Concurrent modification + check...",
        "[*]".cyan().bold()
    );
    ...
    let (status2, body2) = match resp2 {
        ...
        Err(_) => {
            println!("{} Final check failed.", "[-]".red().bold());
            return Ok(());
        }
    };
    println!(
    ...
    println!("\n{} Results:", "[*]".cyan().bold());
    println!(
        "  {} Concurrent modifications accepted: {}",
        "*".cyan(),
        concurrent_success
    );
    println!(
    ...
    if state_changed && concurrent_success > 1 {
        println!(
            "\n{} [HIGH] TOCTOU vulnerability — state changed during concurrent access!",
            "[!]".red().bold()
        );
    } else {
...
pub async fn balance(
    ...
    if success > 1 {
        println!(
            "  {} [HIGH] {} transfers succeeded — double-spend possible!",
            "[!]".red().bold(),
            success
        );
        ...
        println!(
            "  {} Only 1 transfer succeeded — server may have protection.",
            "[-]".yellow().bold()
        );
    } else {
...
pub async fn coupon(
    ...
    let applied = results
        .iter()
        .filter(|(_, b)| b.contains("applied") || b.contains("discount") || b.contains("success"))
        .count();
    ...
    if applied > 1 {
        println!(
            "\n{} [HIGH] Coupon applied {} times — coupon abuse possible!",
            "[!]".red().bold(),
            applied
        );
    } else if applied == 1 {
        println!(
            "\n{} Coupon applied once — server may have protection.",
            "[-]".yellow().bold()
        );
    } else {
