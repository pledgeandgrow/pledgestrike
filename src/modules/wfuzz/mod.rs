...
const PARAM_WORDLIST: &[&str] = &[
    "id", "user", "admin", "debug", "test", "cmd", "exec", "file", "path", "url", "redirect",
    "next", "return", "callback", "token", "key", "secret", "api", "query", "search", "filter",
    "sort", "order", "page", "limit", "action", "type", "name", "value", "data", "input", "output",
    "config", "settings", "env", "var", "flag", "mode", "status", "state", "level", "role",
    "group", "perm", "access", "auth", "session", "csrf", "xss", "sql", "cmd", "shell", "upload",
    "download", "import", "export", "backup",
];
    ...
    for word in &words {
        ...
        if let Ok(resp) = client.get(&test_url).send().await {
            ...
            if status_diff || len_diff || body_diff {
                let tag = if status_diff {
                    "STATUS CHANGE".red().bold().to_string()
                } else if len_diff {
                    "SIZE CHANGE".yellow().to_string()
                } else {
...
pub async fn header(url: &str, wordlist: Option<&str>, timeout: u64) -> anyhow::Result<()> {
    ...
    let (base_status, base_len) = match baseline {
        ...
        Err(_) => {
            println!("{} Baseline failed", "[-]".red().bold());
            return Ok(());
        }
    };
    println!(
    ...
    for word in &words {
        if let Ok(resp) = client.get(url).header(word, "127.0.0.1").send().await {
            let status = resp.status().as_u16();
            let body = resp.text().await.unwrap_or_default();
            let len = body.len();
            let status_diff = status != base_status;
...
pub async fn body(url: &str, wordlist: Option<&str>, timeout: u64) -> anyhow::Result<()> {
    ...
    if interesting.is_empty() {
        println!(
            "\n{} No interesting body params found.",
            "[-]".yellow().bold()
        );
    } else {
        println!(
            "\n{} {} interesting param(s) found:",
            "[*]".cyan().bold(),
            interesting.len()
        );
...
pub async fn cookie(url: &str, wordlist: Option<&str>, timeout: u64) -> anyhow::Result<()> {
    ...
    for word in &words {
        if let Ok(resp) = client
            .get(url)
            .header("Cookie", format!("{}=test", word))
            .send()
            .await
        {
            let status = resp.status().as_u16();
