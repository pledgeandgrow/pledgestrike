...
pub async fn scan(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    println!("\n  {} {} debug endpoints found", "[*]".cyan().bold(), found);
...
pub async fn trace(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    match client.request(reqwest::Method::TRACE, url).send().await {
        Ok(r) => {
            ...
            let text = r.text().await.unwrap_or_default();
            if status == 200 {
                println!("  {} TRACE method enabled — request reflected", "[!]".red().bold());
    ...
    for method in &methods {
        match client.request(reqwest::Method::from_bytes(method.as_bytes()).unwrap(), url).send().await {
            Ok(r) => {
                let status = r.status().as_u16();
                let allow = r.headers().get("allow").map(|v| v.to_str().unwrap_or("")).unwrap_or("");
                if status == 200 || !allow.is_empty() {
                    println!("  {} {:10} — status={} allow={}", "[+]".green().bold(), method, status, allow);
                }
            }
            Err(_) => {}
        }
...
pub async fn stack(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let client = build_client(timeout);
    let trigger_paths = [
        "/error", "/exception", "/throw", "/crash",
        "/nonexistent", "/null", "/undefined",
        "/api/v1/error", "/api/error", "/500",
    ];
    ...
    for path in &trigger_paths {
        ...
        match client.get(&target).send().await {
            Ok(r) => {
                let status = r.status().as_u16();
                let text = r.text().await.unwrap_or_default();
                if text.contains("StackTrace") || text.contains("at java.") || text.contains("at org.") || text.contains("Traceback") {
                    println!("  {} {:20} — STACK TRACE EXPOSED (status={})", "[!]".red().bold(), path, status);
                } else if text.contains("Exception") || text.contains("Error") {
                    println!("  {} {:20} — error info leaked (status={})", "[+]".green().bold(), path, status);
                }
            }
            Err(_) => {}
        }
...
pub async fn source(url: &str, timeout: u64) -> anyhow::Result<()> {
    ...
    let source_paths = [
        ...
        "/archive.zip", "/dist.zip", "/release.zip",
    ...
    for path in &source_paths {
        ...
        match client.get(&target).send().await {
            Ok(r) => {
                ...
                let text = r.text().await.unwrap_or_default();
                if status == 200 && !text.is_empty() {
                    println!("  {} {:30} — {} bytes", "[!]".red().bold(), path, text.len());
                    found += 1;
                }
            }
            Err(_) => {}
