...
pub async fn spray(
    ...
    let users: Vec<String> = std::fs::read_to_string(users_file)?
        .lines()
    ...
    for user in &users {
        let resp = client
            ...
            .body(format!("username={}&password={}", user, password))
            .send()
            .await;
        match resp {
            Ok(r) => {
                ...
                let success = status == 200
                    || status == 302
                    || (status == 401 && !body.contains("invalid") && !body.contains("incorrect"));
                if success {
                    println!("{} [VALID] {}:{}", "[+]".green().bold(), user, password);
                    found.push(user.clone());
                } else {
                    println!("  {} {} — failed ({})", "*".cyan(), user, status);
                }
            }
            Err(_) => {
                println!("  {} {} — error", "*".red(), user);
            }
        }
        if delay > 0 {
...
pub async fn lockout(url: &str, user: &str, timeout: u64, count: u32) -> anyhow::Result<()> {
    ...
    for i in 1..=count {
        let resp = client
            .post(url)
            .header("Content-Type", "application/x-www-form-urlencoded")
            .body(format!("username={}&password=wrong{}", user, i))
            .send()
            .await;
        match resp {
            Ok(r) => {
                let status = r.status().as_u16();
                let body = r.text().await.unwrap_or_default();
                let is_locked = body.contains("locked")
                    || body.contains("disabled")
                    || body.contains("account")
                    || status == 423;
                let is_warned = body.contains("remaining")
                    || body.contains("attempts left")
                    || body.contains("locked out");
                let tag = if is_locked {
                    "LOCKED".red().bold().to_string()
                } else if is_warned {
                    "WARNED".yellow().to_string()
                } else {
                    "ok".to_string()
                };
                println!(
                ...
                if is_locked {
                    locked = true;
                    println!(
                        "{} [INFO] Account locked after {} attempts",
                        "[!]".yellow().bold(),
                        i
                    );
                    break;
                ...
                if is_warned {
                    println!("    {} Warning message detected", ">".yellow());
                }
            }
            Err(_) => {
                println!("  {} Attempt {}/{} error", "*".red(), i, count);
            }
        }
    }
    ...
    if !locked {
        println!(
            "\n{} No lockout detected after {} attempts.",
            "[-]".yellow().bold(),
            count
        );
    }
    ...
    for (name, pwd) in &probes {
        ...
        match resp {
            Ok(r) => {
                let status = r.status().as_u16();
                let body = r.text().await.unwrap_or_default();
                let rejected = body.contains("too short")
                    || body.contains("complexity")
                    || body.contains("require")
                    || body.contains("policy")
                    || status == 400;
                let tag = if rejected {
                    "REJECTED".yellow().to_string()
                } else {
        ...
        for user in &users {
            ...
            match resp {
                Ok(r) => {
                    ...
                    if success {
                        println!("{} [VALID] {}:{}", "[+]".green().bold(), user, pwd);
                        found.push((user.clone(), pwd.to_string()));
                    } else {
                        print!("  {} {} ", "*".dimmed(), user);
                    }
                }
